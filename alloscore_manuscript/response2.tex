...
%DIF ADD manuscript.tex             Sat Aug 31 11:59:01 2024
...
\usepackage{cases}
\usepackage{caption}
\usepackage[colorlinks=true, linkcolor=magenta, citecolor=black]{hyperref}
\usepackage{xurl}
...
to revise and resubmit the manuscript.  Below, we try to provide responses to each point of concern or inquiry from the reviewers.
Text from the reviewers is marked by a left-hand bar and our responses follow inline. We refer to changes in the manuscript
using {\color{magenta}magenta} hyperlinks, which jump to relevant points of a revision-marked manuscript which is included below the response.
Additions appear there in blue with wavy underlining (or inside of a blue box in
the case of the added figure) and removed text appears in red and is struck through.
...
joint probabilities we added a paragraph \hyperlink{diff:pg8_para4_joint_disc}{after paragraph 4 on page 8} 
at the end of Section \ref{sec:methods.related}.
We also renamed Section \ref{sec:methods.related} to better describe these considerations.
...
\hypertarget{point1}{\textbf{[author response point 1]}} We agree that demonstrating relative robustness to assumptions is crucial for promoting practical use of the AS, and had this in mind when writing
Section \ref{sec:sensitivity}, and especially when composing \hyperref[fig:multi-k]{Figure 6}.  The section title, however, only mentioned the integrated AS. To better draw attention to
the issue of robustness we have split the section into two sections, \ref{sec:sensitivity} and \ref{sec:ias_examp}.
and begin the title of Section \ref{sec:sensitivity} with ``Sensitivity''.

We see \hyperref[fig:multi-k]{Figure 6A} as demonstrating not only how AS ranks will tend to be stable across a wide range of constraints but 
...
These changes have been adopted \hyperlink{and1}{here} and \hyperlink{and2}{here}.
...
We have reordered the \hyperlink{refs}{references} alphabetically by lead author last names
...
that any forecast evaluation results using the AS should be accompanied by explanations of how the corresponding resource constraint levels were chosen 
(or weighted in the case of the IAS).  On this issue please see \hyperlink{point1}{author response point 1} above and the changes involving sections \ref{sec:sensitivity} and \ref{sec:ias_examp}
it discusses.  We see 
Figure \ref{fig:multi-k} as an efficient means of highlighting how the ranking and relative differences in scores of the forecasts in this example were in fact stable across a
...
\hyperlink{point1}{author response point 1} we would argue that it is more a feature than a bug of the AS that is becomes less meaningful 
(and less coherent as seen in the tails in figure \hyperref[fig:multi-k]{Figure 6A}) under extreme scarcity or abundance.
...
(as illustrated in sections \hyperref[sec:sensitivity]{3.5.4/5}). Thus fluctuating realized resource constraints would not, in themselves, present a computational barrier. And as
...
That said, sequential decision-making would introduce other complexities that are not captured by
the formulation of the AS introduced in this paper, and we have added a \hyperlink{sequential}{new paragraph} (beginning with ``Secoondly'') to the discussion
...
We have rewritten \hyperref[sec:a:numeric]{Appendix Section C} and created a \hyperref[fig:bisection]{new figure} in order to better convey the simplicity of our numerical methods 
...
We also note that in order to refer to the closed form of the allocation levels for the examples of \hyperref[examp1]{section 2.1} we numbered the \hyperref[eqn:exp-quantile]{relevant formula} in 
\hyperref[sec:a:bayes-quantiles]{Appendix section B}.
...
the kind of \textit{ad hoc} approximation we use in (what is now) section \ref{sec:ias_examp}.  To clarify this we have added text after our \hyperref[eqn:IAS_definition]{definition of the IAS}.
We also added some clarifying text to section \ref{sec:ias_examp} to indicate how an approximation to the IAS was computed in our example.
...
In order to be more transparent regarding the scope of this paper we have rewritten \hyperlink{newdiscussion}{part of the discussion section} to collect limitations mentioned earlier 
(such as those relating to vaccines in \hyperlink{vaxpara}{paragraph 2 of section 2.2.2}) and give some brief methodological suggestions on how they could be confronted.
...
Some of the complexities the referee mentions were noted in section \ref{sec:methods.detailed.specific_allocation}, where we first introduce the loss function:
...
and we have therefore added an additional mention of these possible extensions in \hyperlink{sequential}{paragraph 7} of the revised discussion section
(which begins with ``Secondly, our formulation of the AS...'').
...
We have added several references to the end of the \hyperref[sec:discussion]{first paragraph in the discussion section}, indicating other fields where decision making based on forecasts 
might benefit from the perspective we develop.
We also sought a more general tone in our \hyperlink{lastpara}{concluding paragraph} by addressing it to decision-makers rather than just public health officials.
...
We have tried to offer some initial clarification on the very important issue above in \hyperlink{point1}{author response point 1}, but the referee's question is a broad one
...
a bit more attention this strength we have rephrased the \hyperlink{strengthpoint}{third paragraph} of the introduction.
The ability that the AS gives decision-makers to contextualize differences in forecaster scores is also now mentioned in the 
\hyperref[sec:discussion]{first paragraph of the discussion section}.

\clearpage % Ensure the preface ends and the main article starts on a new page

% Main Article
\pagenumbering{arabic} % Reset to Arabic numerals
...
\citep{colon-gonzalez_probabilistic_2021}. \hypertarget{and1}{ \DIFdelbegin \DIFdel{And }\DIFdelend \DIFaddbegin \DIFadd{Likewise, }\DIFaddend }the WIS has been used during the COVID-19 pandemic to evaluate forecasts
...
literature they represent, }\DIFdelend \DIFaddbegin \DIFadd{they lack }\DIFaddend explicit connections between how a forecast \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend evaluated and how that forecast was used in practice.
\DIFaddbegin \DIFadd{\hypertarget{strengthpoint}{A key impetus} for the present work was to develop a score that is interpretable to decision makers by measuring the extent to which forecasts lead to improved decisions. In addition, the allocation score we propose is expressed in units that are intelligible to decision makers.
}\DIFaddend 
...
impact on and through policy.  \hypertarget{and2}{\DIFdelbegin \DIFdel{And }\DIFdelend \DIFaddbegin \DIFadd{Even }\DIFaddend within this body of work we have found the discussion of such a connection to
...
\hypertarget{vaxpara}{It is helpful to clearly} distinguish between the time $t_d$ when a decision is made about a public health
...
distribution $p$:
\DIFdelbegin \begin{displaymath}\DIFdel{S_{IAS}(F, y) = \int S_A(F,y; K) p(K) \, dK}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{S_{IAS}(F, y) = \int S_A(F,y; K) p(K) \, dK \label{eqn:IAS_definition}
...
\DIFaddbegin \DIFadd{In practice, it may be challenging to compute the integral in \eqref{eqn:IAS_definition}.
...
\DIFaddend \subsection{\DIFdelbegin \DIFdel{Connections to }\DIFdelend \DIFaddbegin \DIFadd{Comparisons with }\DIFaddend Other Scores}
...
\hypertarget{diff:pg8_para4_joint_disc}{
...
\DIFaddend \section{Evaluating forecasts of COVID hospitalizations using the allocation score}
...
%DIF <  Thu Aug  1 13:13:10 2024
%DIF >  Sat Aug 31 11:58:55 2024
\begin{table}[ht]
\centering
\begin{tabular}{lrrrr}
  \hline
...
% latex table generated in R 4.4.1 by xtable 1.8-4 package
%DIF <  Thu Aug  1 13:13:14 2024
%DIF >  Sat Aug 31 11:59:00 2024
\begin{table}[ht]
\centering
\begin{tabular}{lrrr}
  \hline
...
to be a substantial interval around the observed value over which ranks are constant.

\DIFaddbegin \subsubsection{\DIFadd{Integrated allocation score across values of K}}
\label{sec:ias_examp}
...
\DIFaddbegin \DIFadd{With these weight distributions, the IAS defined in Equation~\eqref{eqn:IAS_definition} can be computed as the sum
}$$\DIFadd{S_{IAS}(F, y) = \sum_K S_A(F,y; K) p(K)}$$

\DIFaddend Both versions of the IAS were correlated with the original AS for $K=15,000$, with
...
can be clearly juxtaposed with standard accuracy measures of the forecast \mbox{%DIFAUXCMD
\citep{bannigidadmath2016stock, zhang2019forecasting}}\hskip0pt%DIFAUXCMD
, resource allocation in urban planning \mbox{%DIFAUXCMD
\citep{burnett2014urban, liang2013resource}}\hskip0pt%DIFAUXCMD
, or sustainability initiatives such as water and energy allocation \mbox{%DIFAUXCMD
\citep{syme1999defining, gebre2021multi, colett2016using}}\hskip0pt%DIFAUXCMD
.
}
...
can be clearly juxtaposed with standard accuracy measures of the forecast }\DIFdelend \citep{leitch1991economicForecastEval,
...
directly rewarded by the WIS \DIFaddbegin \DIFadd{(see the discussion at the end of Section \ref{sec:methods.related})}\DIFaddend . 
...
\hypertarget{newdiscussion}{There} are \DIFdelbegin \DIFdel{several }\DIFdelend \DIFaddbegin \DIFadd{three }\DIFaddend important limitations to the current work \DIFdelbegin \DIFdel{. The allocation score we developed here }\DIFdelend \DIFaddbegin \DIFadd{which we noted when introducing the AS in 
...
$K$ by shifting funding from other }\DIFdelend \DIFaddbegin 

\hypertarget{sequential}{}\DIFadd{Secondly, our formulation of the AS does not take into
consideration the temporal inter-dependencies between sequential decisions that public health decision makers are likely to face as an epidemic unfolds. Such dependencies could arise from time-varying inputs such as resource availability or the current effective allocations of resources. The sequential decision problem could also be combined with others allowing constraints for different resources to be modified by time-varying funding shifts between various }\DIFaddend disease mitigation measures.  \DIFdelbegin \DIFdel{Finally, }\DIFdelend \DIFaddbegin \DIFadd{Incorporating these possibilities into an AS framework would likely require technical developments involving either dynamic programming 
(see e.g., \mbox{%DIFAUXCMD
\cite{bertsekas2012dynamic}}\hskip0pt%DIFAUXCMD
) or a scenario-based approach (see e.g., \mbox{%DIFAUXCMD
\cite{pflug2014multistage}}\hskip0pt%DIFAUXCMD
).
}
...
\DIFdelbegin \DIFdel{Our
framework would need considerable enhancements before being applicable }\DIFdelend \DIFaddbegin \DIFadd{For example, this excludes the important problem of allocation of doses of a preventative vaccine.
We have not developed a formal approach }\DIFaddend to forecast evaluation \DIFdelbegin \DIFdel{beyond horizons for which
causal feedback can be neglected}\DIFdelend \DIFaddbegin \DIFadd{using scores given by \eqref{eqn:AS-orc} when both
...
number of cases or deaths averted from one or another allocation strategy}\DIFaddend .

\DIFdelbegin \DIFdel{An }\DIFdelend \DIFaddbegin \DIFadd{Another }\DIFaddend opportunity for further investigation is to more carefully evaluate whether forecasts add value to 
...
for which they are able to offer useful guidance. \hypertarget{lastpara}{New} collaborative work between \DIFdelbegin \DIFdel{public health officials }\DIFdelend \DIFaddbegin \DIFadd{decision-makers }\DIFaddend and modeling teams is
...
$\tau^{\star}$ solves the equation \eqref{eqn:quantiles-sum-to-K}. \DIFdelbegin \DIFdel{However, this }\DIFdelend \DIFaddbegin \DIFadd{This }\DIFaddend equation is not analytically tractable \DIFdelbegin \DIFdel{and we must resort to a numerical method for finding an approximation $\tilde{\tau}$ of $\tau^{\star}$}\DIFdelend \DIFaddbegin \DIFadd{in general, but because the left-hand side of \eqref{eqn:quantiles-sum-to-K} is non-decreasing in $\tau$, it is straightforward to find arbitrarily accurate approximations to a solution using an iterative bisection method}\DIFaddend .

We have
implemented \DIFdelbegin \DIFdel{an iterative bisection method that makes use of the fact that $\sum_{i=1}^N q_{\tau,F_i}$ is an
increasing function of $\tau$.
The algorithm }\DIFdelend \DIFaddbegin \DIFadd{such a method along with the resulting score computations in the R package }{\color{blue}%DIFAUXCMD
\verb`alloscore`
%
}%DIFAUXCMD
\DIFadd{\mbox{%DIFAUXCMD
\citep{gerding-alloscore} }\hskip0pt%DIFAUXCMD
which provided all allocation score values used in the analysis of section
\ref{sec:application}.
The procedure }\DIFaddend begins with an initial search interval $[\tau_{L,1}, \tau_{U,1}]$ (such as
...
%DIFDELCMD < \verb`alloscore`
%DIFDELCMD < %%%
\DIFdel{\mbox{%DIFAUXCMD
\citep{gerding-alloscore} }\hskip0pt%DIFAUXCMD
which provided all allocation score values used in the analysis of section
\ref{sec:application}. }\DIFdelend 

\DIFaddbegin \DIFadd{Figure \ref{fig:bisection} illustrates this bisection method in the context of the examples of Section \ref{sec:methods.overview}, where it quickly finds close approximations to the analytic solutions $\tau^{\star} = 1-e^{-1}, 1-e^{-2}$ (for $K= 5,10$ respectively) which, in this case, are available (cf. \eqref{eqn:exp-quantile}).
}

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[H]
\includegraphics[width=\maxwidth]{figure/bisection-1} \caption{\DIFaddFL{Illustrations of bisection method for the examples of Section \ref{sec:methods.overview}. In each plot the thinner black curves are the quantile functions of the exponential forecasts for Location 1, with $\sigma_1=1$ (lower), and Location 2, with $\sigma_2=4$ (upper), while the thicker black curve gives the sum of these quantiles, which must meet the resource constraints of $K = 5$ on the left and $K=10$ on the right. The red segments show the iterates $\tau_{M,j}$ and their associated total resource demands as given by the function }\texttt{\DIFaddFL{allocate}} \DIFaddFL{from the package }\texttt{\DIFaddFL{alloscore}} \DIFaddFL{applied to these examples and using an initial search interval $[0,1]$. (The $\tau_{M,j}$ are in this case just sums of terms $\pm 2^{-j}$.) The horizontal dashed lines show the resulting allocations (cf. Figure \ref{fig:exp_alloc_example} ).}}\label{fig:bisection}
\end{figure}

\end{knitrout}

\DIFaddend Subtleties can arise when the forecast densities $f_i$ vanish or are very small, in which case quantiles are non-unique
