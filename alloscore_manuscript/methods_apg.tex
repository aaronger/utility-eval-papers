...
$\{i \mid x_i^{F,k} > 0\}$ and a root $\lambda^{\star}$, in terms of which we can exrpess the Bayes act as
\begin{align}
x_i^{F,k} = 
\begin{cases}
...
for $\lambda^{\star}$ under the assumption that $I_+ = \{1,\ldots,N\}$.\footnote{Such an assumption would exclude fairly simple examples in which $F$ has support bounded away from $0$ in one coordinate but not in another.} 
...
our original objective function $E_F\left[s_A(Y,x)\right]$ is decomposable in the coodinates $x_i$, allowing the corresponding infima in $\mathcal{L}_D$ to be computed separately:
\begin{align}
