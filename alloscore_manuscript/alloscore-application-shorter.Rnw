...
In the wake of the COVID-19 pandemic, researchers have been calling for evaluating infectious disease forecasts in ways that align more specifically with how forecasts can be used to inform policy decisions \citep{marshall2023predictions, bilinski_adaptive_2023}.
Historically, the quality of disease forecasts has typically been measured using scores such as root mean square error (RMSE) \citep{papastefanopoulos2020covid}, log score \citep{mcgowan_collaborative_2019}, or weighted interval score (WIS) \citep{bracher2021evaluating}.
These scores can be interpreted as measuring how well the forecast corresponds to the observed disease outcome but often do not clearly measure the value of a forecast in the context of a specific decision, a result that is well known from other fields, such as economics and finance, supply chain management, and meteorology.
However, as we show in this paper, it is possible to construct formal methods for evaluating forecasts that are directly and uniquely tied to policy decisions (in our hypothetical setting, a question of resource allocation).
Furthermore, such scores can illuminate important features of forecasts, and their value as decision-making inputs, that can be missed by traditional accuracy measures.
...
%There is a large history of literature applying these ideas to obtain measures of the value of forecasts that are tied to a decision making context, primarily in fields such as economics and finance, supply chain management, and meteorology.
...
For example, seasonal influenza forecasts in the US and dengue forecasts for Peru and Puerto Rico targeted public health surveillance measures of incidence over time and space, and used log score and mean absolute error to evaluate forecast skill \citep{mcgowan_collaborative_2019,reich_collaborative_2019,johansson_open_2019}.
...
However, the framework described in this paper corresponds to real-world decisions that must be made by public health administrators around the globe, and could be adapted in the future for real-time situations such as informing bed demand, ventilator usage, or doses needed for specific medications.
