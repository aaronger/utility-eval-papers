...
\end{enumerate}
This is a general procedure that may be applied in settings where it is possible to specify a quantitative loss function. Subject to certain technical conditions, scoring rules obtained from this procedure are proper. We refer the reader to the supplement for a more technically precise discussion.
...
When the oracle has sufficient resources to meet the total need, i.e., when $\sum_{i=1}^{N}y_i \leq K$, the oracle's loss is zero and there is no adjustment.  Thus the allocation score coincides with
the raw score in that case.
...
The oracle-adjusted score is
\begin{align}
S_A(F, y; K)  &= S_A^{\text{raw}}(F, y; K) - l_{\text{oracle}}(y;K) \\
&= \begin{cases}
S_A^{\text{raw}}(F, y; K), &  \sum_{i=1}^{N}y_i \leq K \\
S_A^{\text{raw}}(F, y; K) - L \cdot \left( \sum_{i=1}^{N}y_i - K\right), &  \sum_{i=1}^{N}y_i > K.
\end{cases}
\end{align}
...
The hospitalization data used for scoring forecasts were downloaded on \Sexpr{format(as.Date(metadata$time[which(metadata$name == "truth_data")]), "%B %d, %Y")}.

\subsection{Evaluation metrics}

We measured forecast skill using two forecast scores, the allocation score and the weighted interval score (WIS), both defined above.
We computed these scores for the 14 day ahead forecasts made each week.
For this analysis, we fixed the resource constraint used for the allocation score to be $K=15,000$, based roughly on a reported number of ventilators available for reallocation in the US \citep{ajao_assessing_2015}.
...
Often, these scores are used to rank models by accuracy according to that particular score, but without reference to the underlying decision-making process for which that score was designed.
...
Those that can be easily quantified may differ enough that no single score would be appropriate for all users.
