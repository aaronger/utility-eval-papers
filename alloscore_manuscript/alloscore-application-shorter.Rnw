...
Our example application of the AS refers to a hypothetical and unspecified resource need that is supposed to 
pair with our forecasted outcome (new hospital admissions) in a manner that might appear overly simplistic and 
artificially direct.
A concrete decision problem, which motivated our work on allocation scoring, is

allocation of ventilators during a respiratory viral pandemic
\citep{huang_stockpiling_2017}.
