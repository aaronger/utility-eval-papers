...
In the wake of the COVID-19 pandemic, researchers have been calling for evaluating infectious disease forecasts through their impact on policy \citep{marshall2023predictions, bilinski_adaptive_2023}.
Historically, the quality of disease forecasts has typically been measured using scores such as root mean square error (RMSE) \citep{papastefanopoulos2020covid}, log score \citep{mcgowan_collaborative_2019}, or weighted interval score (WIS) \citep{bracher2021evaluating}.  These scores can be interpreted as measuring how well the forecast corresponds to the observed disease outcome.
However, as we show in this paper, these scores often do not clearly measure the value of a forecast in the context of a specific decision.
