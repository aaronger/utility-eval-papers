...
However, we acknowledge the limitations of this example in that not everyone who is hospitalized needs a ventilator, and new hospital admissions may not be the best stand in for a need that corresponds to a prevalence of severe cases.
...
While there are limitations in the realism of the pairing of a specific resource and forecasted outcome, we emphasize that the important result from this work is how evaluating forecasts using a fundamentally different kind of score can illuminate previously hidden aspects of value in forecasts.
