...
To adequately compare allocations from individual models with existing standard practices, we used two benchmarks against which the performance of using forecasts to inform allocations could be evaluated.
First, we evaluated forecasts from the \texttt{COVIDhub-baseline} model, which predicts a flat line from the most recent observation with uncertainty bounds based on a random walk, and the allocations that resulted from those forecasts \citep{cramer_evaluation_2022}.
Second, we generated a proposed set of allocations on a per capita basis by state (using US Census data of vintage 2022 \citep{us_census_nst_est2022}), referred to below as \texttt{per-capita}.
...
Averaging across all weeks, improvements in mean allocation score (MAS) were observed when comparing the ensemble forecast to two benchmark comparisons.
...
AS was computed for a range of $K$ from 200 to 60,000 for forecasts made on December 20, 2020 predicting levels of hospitalizations on January 3, 2021 as well as the per-capital allocation (Figure \ref{fig:multi-k}A).
