...
We refer to a realization $y$ of $Y$ as an \emph{outcome}. When a decision maker assumes $Y$ to have a specified
distribution $F_Y$ and $Y$ is the only aspect of the future relevant to the selection of $x$, the process is referred to
as a decision under risk.  The selection of $x$ without a full specification of the distribution of $Y$ is referred to
as a decision under uncertainty. Clearly almost any real-world decision will be a decision under uncertainty.  Yet many
foundational results in decision theory were guided (as we will be) by the intuition that much can be learned about a
decision making process by building a ``small world'' approximation of the real-world (after \cite{savage1954foundations})
in which the beliefs of a decision
maker are equivalent to a fully specified distribution of a $Y$ that exhaustiveley quantifies all relevant aspects of
the future, converting a decision under uncertainty to a decison under risk.

Examples of actions with which this paper is concerned are levels of investment in measures designed to mitigate severe
disease outcomes such as hospital beds, ventilators, medication, or medical staff, with $\mathcal{X}$ being the set of
all feasible levels of investment. An outcome $y$ determining the consequences of such an action might be the number of
individuals who eventually become sick and would benefit from the mitigation measure. A fundamental strategy in decision
theory is to assume that the consequences of an action $x$ under outcome $y$ can be assigned a numerical value, or
\emph{loss}, measuring the (lack of) success of $x$. In our example, $x$ will succeed to the extent that it meets the
realized need $y$, so that greater unmet need will incur greater loss. 

One tool a decision maker can use to anticipate the consequences of an action $x$ is a forecast
$F$ of $Y$. And within a ``small world'' where $Y$ exhaustively quantifies the future, the decision maker might choose that
$x$ which yields the lowest expected loss according to $F$. The loss of the action $x$ chosen according to $F$ can then, 
in turn, be interpreted as the value of $F$ as an
input to the decision making process under the realized outcome $y$ of $Y$.

Through this discussion, we arrive at a three-step procedure for developing scoring rules for probabilistic forecasts:
\begin{enumerate}
...
function. We call a scoring rule obtained from this procedure a \emph{Bayes scoring rule} (noting that to our knowledge, this terminology is not standard).  In appendix \ref{sec:proper} we review the
...
and the allocation score are equivalent in the sense that they will yield the same score for any distribution $F$.
