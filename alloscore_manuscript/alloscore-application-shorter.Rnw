...
Decision theory is concerned with how to decide on an \emph{action} $x$ to select from a set of
possible actions $\mathcal{X}$ in the face of uncertainty about the future.
We use the random variable $Y$ to represent future states and refer to a
realization $y$ of $Y$ as an \emph{outcome}. For example, $x$ might correspond to the level of investment in a measure that is
designed to mitigate severe disease outcomes such as hospital beds, ventilators, medication, or medical staff, with
$\mathcal{X}$ being the set of all feasible levels of investment, while an outcome $y$ might correspond to the number of
individuals who eventually become sick and would benefit from the mitigation measure. A foundational strategy in decision
theory is to assume that the consequences of an action $x$ under outcome $y$ can be assigned a numeric value, or
\emph{loss}, measuring the (lack of) success of $x$. In our example, the loss measures the extent to which $x$ meets the realized need
$y$, with greater unmet need incurring a larger loss.
One way that a decision maker can try to anticipate the consequence of an action $x$ is by using a forecast $F$ of
$Y$.
This can help inform the selection of the action to take; for example, the decision maker might choose an action with the lowest expected loss according to the forecast.
The loss of the action $x$ chosen according to $F$ can, in turn, be interpreted as the value of $F$ as an input to
