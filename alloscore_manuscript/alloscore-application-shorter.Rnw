...
We begin with an informal description of the allocation score and some examples illustrating its key characteristics in section \ref{sec:methods.overview}. In section \ref{sec:methods.detailed} we develop the allocation score more carefully, using a basic decision theoretic procedure for deriving proper scoring rules. (See section \ref{sec:proper} of the supplement for a definition of a proper scoring rule
...
supplement we show that they are proper by construction.
