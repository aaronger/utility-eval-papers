...
unmet in location 2. But these 6 units of unmet need are unaviodable under the constraint $K=5$: allocating 0 units of
...
A state-wise comparison using Figures \ref{fig:thermometer-plot} and \ref{fig:multi-loc-ranks}A of the
...
model were less efficient than those of the \texttt{CU-select} model. Thus \texttt{JHUAPL-Bucky} achieved a worse
...
reduced unmet need.  These allocation errors of \texttt{JHUAPL-Bucky} resulted from forecasts failing to consistently
...
(a) has been identified as a prime manifestation of scarcity during respiratory outbreaks (b) tansparently involves
...
hospitalized needs a ventilator, and new hospital admissions will not fully describe for a need that corresponds to a
prevalence of severe cases. Other possible real-world examples include the allocation of a limited stockpile of
...
reallocation increases the expected shortage in location $i$ is at least as much as it decreases the expected shortage
in location $j$:
\begin{align*}
...
In section \ref{sec:application} of the manuscript, we used the allocation score to evaluate forecasts of COVID-19
