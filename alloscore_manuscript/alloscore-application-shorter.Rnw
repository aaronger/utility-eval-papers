...
14 days. For this analysis, we fixed the resource constraint used for the AS to be $K=15,000$, based roughly on a
reported number of ventilators available for reallocation in the US \citep{ajao_assessing_2015}. We computed the mean
...
\citep{araz_geographic_2012,persad_fair_2023} or diagnostic tests \citep{du_optimal_2022,pasco_covid-19_2023}. While
...
In practice, there are many potential users of forecasts with many different decision making problems. While outcomes of
some decisions could potentially be specified by loss functions, other uses for forecasts may be less quantifiable, such
as when forecasts are used to inform a general sense of situational awareness that may feed in unmeasurable ways into
other decisions. Even those that are easily framed as expected loss minimization may differ enough that no single score
...
the set of scoring methods used by the hub to include scores based on specific public health decision problems, such as
AS, allowing end users to better understand the value of available forecasts as inputs to their particular
