...
\subsection{Propriety -- prospective}
...
the distribution corresponding to the optimal submission for the forecaster with predictive distribution $F$.
Because the distribution $G^q(F)$ typically will not yield allocations that are optimal under the distribution $F$, in general $G^q(F)$ and $G^\star(F)$ are different distributions.
