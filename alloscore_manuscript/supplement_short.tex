...
That is,
\begin{align}
E_{F}[\tilde{S}(G^\star(F), Y)] &< E_{F}[ S_A(G^q(F), Y) ] \label{eqn:tilde_s_improper} \\
  &= E_{F}[\tilde{S}(F, Y)] \nonumber
\end{align}
The inequality in step \eqref{eqn:tilde_s_improper} is strict when $G^q(F)$ is not a minimizer of the expected allocation loss under the distribution $F$. At an intuitive level, this occurs when the allocations determined by $G^\star(F)$ and $G^q(F)$ are different.
