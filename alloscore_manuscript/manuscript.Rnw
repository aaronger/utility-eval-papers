...
iters5 <- tibble(taus = 1-alloc$lam_seq[[1]], qtot = q_sum(taus), ind = seq_along(alloc$lam_seq[[1]]))
iters10 <- tibble(taus = 1-alloc$lam_seq[[2]], qtot = q_sum(taus), ind = seq_along(alloc$lam_seq[[2]]))
...
p5 <- p + geom_hline(aes(yintercept = 5)) +
  ...
  geom_segment(data = iters5, aes(x=taus, xend = taus, y=0, yend = qtot, color = ind), alpha = .5) +
  scale_color_viridis_c("Iteration\n index", end = 0.75, direction=-1, limits = c(1, 18), option="B") +
...
p10 <- p + geom_hline(aes(yintercept = 10)) +
  ...
  geom_segment(data = iters10, aes(x=taus, xend = taus, y=0, yend = qtot, color = ind), alpha = .5) +
  scale_color_viridis_c("Iteration\nindex", end = 0.75, direction=-1, limits = c(1, 18), option="B") +
  scale_x_continuous(limits = c(.45,1), expand = c(0,0), 
...
legend <- get_legend(p10 + theme(legend.box.margin = margin(0, 0, 0, 12)))
