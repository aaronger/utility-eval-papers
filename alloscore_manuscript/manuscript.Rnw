...
\begin{align} 
  S_{IAS}(F, y) &= \int S_A(F,y; K) p(K) \, dK \label{eq:IASintegral}\\
    & = \sum_{K=1}^{\infty} S_A(F,y; K) p(K) \quad (\text{assumming non-divisible resource units})\label{eq:IASsum}.
\end{align}
...
Calculating exact IAS as defined in equation \eqref{eq:IASsum} is computationally intensive so an approximate IAS, denoted $\hat{S}(F,y)$, was used such that
\begin{align*}
	\hat{S}(F,y) = \sum_{k=1}^{600} S_A(F,y; 200k) p(200k),
\end{align*}
where summation terms were calculated at every 200 beds with an upper bound of 60,000, or four times the estimated available beds.
An approximate IAS was computed for two weight distributions on $K$, one uniform
...
scoring rules that directly consider the success of forecasts in supporting specific decisions. The allocation score was developed for this explicit purpose, evaluating forecasts in the context of the decisions they lead to. The AS is expressed in units that are directly relevant to decisions, which helps not only with ranking forecasts in the context of a specific action, but also allows decision-makers to contextualize the magnitude of differences between forecasts. The benefits of the work presented here generalize beyond public health, to any application of probability forecasts for decision-making, such as economics, where the monetary consequences of using a forecast
can be clearly juxtaposed with standard accuracy measures of the forecast \cite{bannigidadmath2016stock, zhang2019forecasting}, resource allocation in urban planning \citep{burnett2014urban, liang2013resource}, or sustainability initiatives such as water and energy allocation \cite{syme1999defining, gebre2021multi, colett2016using}.
...
Additionally, during an epidemic, public health decision makers may be faced with an ongoing series of decisions over time in response to changes in conditions such as resource availability or the current effective allocations of resources. The AS presented here looks at a single snapshot in time  and ignores many of the possible complexities related to adjusting existing allocations. The example presented in section 
\ref{sec:application} is based on a prospective estimate of bed availability, but one could also use observed total resource availability to target an optimal set of prospective AS parameters. 
Alternatively, the impact of unmet need often differs across different locations and demographics, which could be accounted for by incorporating a function of location and demographics which defines the cost of unmet need. Future work is needed to explore the use of AS while accounting for the dynamic nature of disease spread and resource availability.
...
Figure \ref{fig:bisection} illustrates this bisection method in the context of the examples of Section \ref {sec:methods.overview}, where it quickly finds close approximations to the analytic solutions $\tau^{\star} = 1-e^{-1}, 1-e^{-2}$ (for $K= 5,10$ respectively) which, in this case, are available (c.f. \eqref{eqn:exp-quantile}).

<<bisection, fig.height=4, fig.width = 8, fig.pos="H", fig.cap = "Illustrations of bisection method for the examples of Section \\ref{sec:methods.overview}. In each plot the thinner black lines are the quantile functions of the exponential forecasts for Location 1, with $\\sigma_1=1$ (lower), and Location 2, with $\\sigma_2=4$ (upper), while the thicker black line gives the sum of these quantiles, which must meet the resource constraints of $K = 5$ on the left and $K=10$ on the right. The red segments show the iterates $\\tau_{M,j}$ and their associated total resource demands as given by the function \\texttt{allocate} from the package \\texttt{alloscore} applied to these examples and using an initial search interval $[0,1]$. (The $\\tau_{M,j}$ are in this case just sums of terms $\\pm 2^{-j}$.) The horizontal dashed lines show the resulting allocations (c.f. Figure \\ref{fig:exp_alloc_example} ).">>=
expexamp <- tibble(
  target_names = LETTERS[1:2],
  dist = "exp",
  rate = c(1,.25),
) %>% add_pdqr_funs(types = c("p", "d", "q"))

alloc <- allocate(df = expexamp, K = c(5,10))

q1 <- expexamp$Q[[1]]
q2 <- expexamp$Q[[2]]
q_sum <- function(tau) q1(tau) + q2(tau)
iters5 <- tibble(taus = 1-alloc$lam_seq[[1]], qtot = q_sum(taus))
iters10 <- tibble(taus = 1-alloc$lam_seq[[2]], qtot = q_sum(taus))

p <- ggplot() +
  geom_function(fun = q1, n=1000) +
  geom_function(fun = q2, n=1000) +
  geom_function(fun = q_sum, n=1000, linewidth = 1) +
  labs(
    x = expression(tau),
    y = expression(paste(italic(q[tau * "," * F[1]]),
                         ", ", 
                         italic(q[tau * "," * F[2]]),
                         "  and  ", 
                         italic(q[tau * "," * F[1]] + q[tau * "," * F[2]])))) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.ticks.length.x = unit(.25, "cm"))

p5 <- p + geom_hline(aes(yintercept = 5)) +
  geom_hline(aes(yintercept = c(1,4)), linetype = 2) +
  geom_segment(data = iters5, aes(x=taus, xend = taus, y=0, yend = qtot), color = "red", alpha = .5) +
  scale_x_continuous(limits = c(.45,1), expand = c(0,0), 
                     breaks = c(.5,.6,0.632,.7,.8,.9,1),
                     labels = c("0.5","0.6",expression(italic(atop(" ", 1 - e^{-1} %~~% 0.632))),
                                "0.7","0.8","0.9","1")) +
  scale_y_continuous(limits = c(0,12), breaks = c(0,1,4,5,10,15), expand = c(0,0))

p10 <- p + geom_hline(aes(yintercept = 10)) +
  geom_hline(aes(yintercept = c(2,8)), linetype = 2) +
  geom_segment(data = iters10, aes(x=taus, xend = taus, y=0, yend = qtot), color = "red", alpha = .5) +
  scale_x_continuous(limits = c(.45,1), expand = c(0,0), 
                     breaks = c(.5,.6,.7,.8,0.865,.9,1),
                     labels = c("0.5","0.6",
                                "0.7","0.8", 
                                expression(italic(atop(" ", 1 - e^{-2} %~~% 0.865)), "0.9", "1"))) +
  scale_y_continuous(limits = c(0,12), breaks = c(0,2,5,8,10,15), expand = c(0,0), 
                     name = element_blank())

cowplot::plot_grid(p5, p10, align = "h")
