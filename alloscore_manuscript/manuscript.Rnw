...
reduce the number of people who will become sick within the forecast horizon. Instead, our setup addresses resources
...
directly rewarded by the WIS (cf. the discussion at the end of Section \ref{sec:methods.related}). 
...
We reiterate three important limitations to the current work which we noted when introducing the AS in 
Section \ref{sec:methods.detailed.specific_allocation}.  We view these issues as promising avenues for 
future investigation, but beyond the scope of this paper.
...
appropriate to make the loss for failing to
procuring a bed for a hospitalizable illness an increasing function of the state's median age. 
...
This excludes the fundamentally important context of vaccine allocation for certain pathogens and
spatio-temporal scales, but we note that supply-chain decisions made at a national level in the US determining the state-level
availability of current COVID-19 vaccines are unlikely to affect incidence or exposure levels at a 14-day horizon. In any event,
we are not yet prepared to suggest how valid forecast evaluation using scores given by \eqref{eqn:AS-orc} would proceed when 
both disease burden and unmet need are considered as endogenous variables.
