...
one uniform across the entire range and the other with weights proportional a truncated normal distribution centered at 15,000
...
disease burden and unmet need are considered as endogenous variables. Our sense is that such an approach would require some form of
accounting for the causal effect of the allocation decision on observed measures of disease burden, e.g. estimating the (counterfactual)
cases or deaths averted from one or another allocation strategy through the available observed data.
