...
%DIF ADD manuscript.tex             Tue Aug 13 15:15:54 2024
...
\DIFdelbegin \DIFdel{IAS was }\DIFdelend \DIFaddbegin \DIFadd{Calculating exact IAS as defined in equation \eqref{eq:IASsum} is computationally intensive so an approximate IAS, denoted $\hat{S}(F,y)$, was used such that
}\begin{align*}
	\DIFadd{\hat{S}(F,y) = \sum_{k=1}^{600} S_A(F,y; 200k) p(200k),
}\end{align*}
\DIFadd{where summation terms were calculated at every 200 beds with an upper bound of 60,000, or four times the estimated available beds.
An approximate IAS was }\DIFaddend computed for two weight distributions on $K$, one uniform
...
\cite{syme1999defining, gebre2021multi, colett2016using}}\hskip0pt%DIFAUXCMD
.
}
...
directly rewarded by the WIS \DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{(cf. the discussion at the end of Section \ref{sec:methods.related}). 
}\DIFaddend 
...
\DIFdelbegin \DIFdel{There are several }\DIFdelend \DIFaddbegin \DIFadd{We reiterate three }\DIFaddend important limitations to the current work \DIFdelbegin \DIFdel{. The allocation score we developed here }\DIFdelend \DIFaddbegin \DIFadd{which we noted when introducing the AS in 
...
\DIFadd{Secondly, our formulation of the AS does not take into
...
causal feedback can be neglected}\DIFdelend \DIFaddbegin \DIFadd{This excludes the fundamentally important context of vaccine allocation for certain pathogens and
...
both disease burden and unmet need are considered as endogenous variables}\DIFaddend .
...
for which they are able to offer useful guidance. New collaborative work between \DIFdelbegin \DIFdel{public health officials }\DIFdelend \DIFaddbegin \DIFadd{decision-makers }\DIFaddend and modeling teams is
