...
left_join(ias, as) |>
  rename(`IAS centered at 15k` = IAS_wt,
         `IAS uniform` = IAS) |>
  arrange(`IAS centered at 15k`) |>
  xtable::xtable(caption = "Allocation scores shown in Figure \\ref{fig:multi-k}C, sorted by Integrated Allocation Scores (IAS) centered at 15,000.",
                 digits = 0,
