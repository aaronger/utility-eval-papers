...
AS was computed for a range of $K$ from 200 to 60,000 for forecasts made on 2020-12-20 predicting levels of hospitalizations on 2021-01-03 (Figure \ref{fig:multi-k}A).
...
IAS was computed for two distributions of $K$, one uniform across the entire range and the other a symmetric distribution around 15,000 (Figure \ref{fig:multi-k}B).
...
In the context of allocation-based decisions, standard procedures might involve simply extrapolating need based on a current observed data (similar to the `baseline' model presented above), with or without adjustments based on other political or real-world considerations.
...
The allocation score presented in this work assesses the optimality of the allocation based on an omniscient view of the future.
