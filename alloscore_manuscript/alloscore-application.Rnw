...
In our context, this result has the interpretation that in order to allocate optimally (according to $F$), we must divide resources among the locations so that there is an equal forecasted probability in every location that the allocation is sufficient to meet resource need.
We note that this probability, $\tau(K,F)$, depends not only on the constraint level $K$ defining the decision problem but also on the forecast $F$.
This is quite different from the situation for quantile scores and the WIS, where the probability levels used to calculate scores are fixed.

A related interpretation comes from the observation that at the allocation given by the Bayes act, the rate of change of the expected loss with respect to the amount of resources allocated to a particular location $i$ is the same for all locations, and is given by
\begin{align}
    \frac{\partial}{\partial x_i} \mathbb{E}_{F} [s_A(x, Y)] \vert_{x=x^{F,K}} = L \cdot (\tau - 1).
\end{align}
Therefore, when our allocations are set to the Bayes act $x^{F,K}$, shifting a small amount of the resource from one location to another would yield no change in total expected unmet need since the change in expected unmet need in the receiving location would be canceled out by the change in the location giving up the resource.
