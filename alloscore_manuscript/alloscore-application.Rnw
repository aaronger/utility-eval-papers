...
In summary, proper scoring rules for probabilistic forecasts can be traced back to a loss function motivated by a specific decision-making context.
Often, forecast scores are used to rank models according to accuracy with that particular score, but without reference to the underlying decision-making process for which that score was designed.
...
In some situations, individual or ensemble models could be developed to minimize the loss for a particular setting, although this is also an area largely unexplored in the realm of public health to date.
