...
Of the 10 models evaluated, the CU-select model had the most accurate forecasts according to the allocation score while the USC-SI\_kJalpha model had the most accurate forecasts based on MWIS (Table \ref{tab:one-week-results}).
...
For JHUAPL-Bucky, the allocations were assigned at the \Sexpr{Buck_quantile}th quantile of the predictive distribution and for CU-select it was at the \Sexpr{CU_quantile}th percentile.
This reflects that in general the JHUAPL-Bucky forecasts assigned more probabilities to higher values and thus a low quantile value across all locations reached the allocation limit of 15,000.
In California, New York and Texas, the eventually observed number of hospital admissions was closer to the predicted median from JHUAPL-Bucky and was contained in JHUAPL-Bucky's 80\% PI. However for each of these three states, the resource allocation was lower and therefore resulted in more hypothetical unmet need than CU-select allocations.
In Florida, the forecast from JHUAPL-Bucky missed by a wide margin (as did the forecast from CU-select) and the allocation was \Sexpr{FL_diff} units lower than what was allocated according to the CU-select model.
...
The integrated allocation score summarizes allocation scores across a range of possible values of the constraint ($K$), possibly taking weights into account for different values of $K$ that might be more or less likely (Section \ref{sec:methods.detailed.integrated_allocation}).
...
Unlike in the above analyses where we conducted analyses assuming that $K$ was known to be 15,000, the analyses in this subsection are conducted assuming different distributions of $K$.
...
Allocation scores (AS) were computed for a range of $K$ from 200 to 60,000 for forecasts made on 2020-12-20 predicting levels of hospitalizations on 2021-01-03 (Figure \ref{fig:multi-k}A).
These calculations highlight that allocation scores were highest at the $K$ nearest to the observed value of \Sexpr{format(tot_y, big.mark = ",")}.
Rankings of allocation scores from all models were fairly stable across a range of values for K, with some crossings, especially at $K$ values further away from the observed value.

Integrated allocation scores (IAS) were computed for two distributions of $K$, one uniform across the entire range and the other a symmetric distribution around 15,000 (Figure \ref{fig:multi-k}B).
...
Interpretation of Figure \ref{fig:multi-k} needed.
...
left_join(ias, as) |>
  rename(`IAS centered at 15k` = IAS_wt,
         `IAS uniform` = IAS) |>
  xtable::xtable(caption = "Allocation scores shown in Figure \\ref{fig:multi-k}C, sorted by Integrated Allocation Scores (IAS) centered at 15,000.", digits = 0) |>
  print(include.rownames = FALSE, )
@



<<multi-k, fig.height=10, fig.width = 8, fig.cap = "Allocation scores across different resource constraints (K) for the 10 models that made forecasts on December 20, 2021. Panel A shows, for each model, the allocation score assigned for values of K between 200 and 60,000 at increments of 200. The allocation scores show a sharp peak just under 20,000 which was the eventually observed number of hospitalizations. It is hardest to get allocations exactly right, with no excess unmet need, when the resource constraint is exactly equal to the number of resources needed. Panel B shows two possible weighting functions for the Integrated Allocation Score (IAS). The first (dark blue circles) computes weights proportional to a normal distribution centered at 15,000 (solid vertical gray line) with a standard deviation of 3,000, and truncated to be between 5,000 and 25,000. The second (light orange triangles) uses a uniform weight for all possible values of K. Note that the Allocation Score (AS), used in earlier sections of the application, only uses the allocation scores at the `pre-defined' value of K=15,000. the  Panel C shows how either method of computing the IAS (y-axis, again in blue circles and orange squares) is correlated with AS (x-axis, computed at K=15,000). Every point represents the AS and IAS for one model. Note that the IAS centered at 15,000 are much more closely correlated  with the AS values, which makes sense as this version of the IAS assigns higher weights to values of the IAS that are closer to the single K at which the AS scores were computed.">>=
