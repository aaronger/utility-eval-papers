...
For JHUAPL-Bucky, the allocations were assigned around the 25th quantile of the predictive distribution and for CU-select it was around the 90th percentile. \ngr{could we make these more specific?}
...
In Florida, the forecast from JHUAPL-Bucky missed by a wide margin (as did the forecast from CU-select) and the allocation was nearly XX units lower than what was allocated according to the CU-select model. \ngr{add specific number here}
