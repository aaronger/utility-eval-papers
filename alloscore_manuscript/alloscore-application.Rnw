...
\ngr{This section label doesn't exist. Should it point to methods.detailed?}
...
Overall, the \texttt{COVIDhub-baseline} model, which predicts a flat line from the most recent observation with uncertainty bounds based on a random walk, had the highest rank for allocation score in six weeks, more than any other model except the \texttt{COVIDhub-ensemble} which also had six.
...
We note that others have explored similar linkages.
For example, \cite{bilinski_adaptive_2023} explore ways in which policymaker preferences could inform risk thresholds for predictive models.
\ngr{Would be nice to have a few other examples here.}
In some situations, individual or ensemble models could be developed to minimize the loss for a particular setting, although this is also an area largely unexplored in the realm of public health to date, with some initial methodological development in econometrics.\citep{loaiza-maya_focused_2021}
