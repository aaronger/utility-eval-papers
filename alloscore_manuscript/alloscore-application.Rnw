...
expected loss under the constraint is that there is no way to decrease expected loss further by shifting a small amount of the resource from one location to another.  As a consequence, at $x^{F,K}$, all the rates of change of expected loss with respect to the amount of resources allocated to a particular location $i$ must be equal:
\begin{align}
 \frac{\partial}{\partial x_i} \mathbb{E}_{F} [s_A(x, Y)] \vert_{x=x^{F,K}} = \lambda
\end{align}
for some constant $\lambda$ that is the same for all locations $i$. It turns out that these derivatives can also be expressed in terms of 
the marginal forecasts $F_i$ as
\begin{align}
...
\end{align}
A solution to this equation---which almost always must be approximated with a numerical method---can be substituted back into the quantile formulas
...
We note that $\lambda = \lambda(K,F)$ and the probability level $1-\lambda/L$ depend on both the constraint level $K$ defining the decision problem and on the forecast $F$. This is quite different from the situation for quantile scores and the WIS, where the probability levels used to calculate scores are fixed.

\paragraph{Step 3: Define the scoring rule.} We can now define a proper scoring rule for the probabilistic forecast $F$ as
\begin{equation}
S_A(F, y; K) = s_A(x^{F,K}, y) = \sum_{i=1}^n L \max(0, y_i - F_i^{-1}(1- \lambda(K,F)/L))
\end{equation}
