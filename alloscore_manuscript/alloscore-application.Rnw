...
\end{align}
where $\mathbb{E}_{F} [s_A(x, Y)] = \sum_{i=1}^{N} L \mathbb{E}_{F_i}[max(0, Y_i - x_i)]$ sums the expected loss due to unmet need across all locations. It can be shown that the Bayes act has $x^F_i = F_i^{-1}(1 - \lambda^{\star}/L)$, where $\lambda^{\star}$ depends on the problem parameters $K$ and $L$ as well as the forecast distributions and is chosen so as to satisfy the equation
\begin{align}
...
\begin{equation}
S_A(F, y; K) = s_A(x^F, y) = \sum_{i=1}^n L \max(0, y_i - F_i^{-1}(1 - \lambda^{\star}/L))
\end{equation}
