...
This also underscores the points above that allocation scores are highest where the observed value is hear the allocation constraint, but here we are holding the date/models fixed and varying K.

Allocation scores are fairly stable across a range of values for K, and therefore IAS is not very sensitive to the choice of distribution on K, and does not differ much from the regular old allocation score as long as that allocation score was computed at a reasonable choice of K.
