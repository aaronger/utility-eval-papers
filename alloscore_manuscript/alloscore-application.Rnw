...
\begin{align}
S_A^{\text{raw}}(F, y; K) = s_A(x^{F,K}, y) = \sum_{i=1}^N L \max(0, y_i - F_i^{-1}(1- \lambda(K,F)/L))
\end{align}
...
forecast performance, we will adjust it by the the minimum loss $l_{\text{oracle}}(y;K)$ achievable by an \emph{oracle} allocator which has precise foreknowledge of the outcomes $y_i$ which is $l_{\text{oracle}}(y;K) = L \max(0, \sum_{i=1}^{N}y_i - K)$.  Our scoring rule is then given by
\begin{align}
S_A(F, y; K) &= S_A^{\text{raw}}(F, y; K) - l_{\text{oracle}}(y;K) \\
& = \sum_{i=1}^N L \max(0, y_i - x_i^{F,K}) - L \max(0, \sum_{i=1}^{N}y_i - K) \\
& = 
\begin{cases}
\sum_{i=1}^n L \max(0, y_i - x_i^{F,K}), &  \sum_{i=1}^{N}y_i \leq K \\
LK - L\sum_{i=1}^{N} \min(x_i^{F,K}, y_i), &  \sum_{i=1}^{N}y_i > K.
\end{cases}
\end{align}
