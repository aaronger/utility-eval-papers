...
to $\tau^{\star}$ from below and above. Note that this algorithm is somewhat more delicate than the usual ``bisection'' algorithm which
would involve bisecting intervals in the domain of $F_T$ to find a root.
...
We also want to be able to decribes what further ``post-processing'' steps are required to find an approximate Bayes act in the set
$\undertilde{\bm{F}}^{-1}(I_{\overline{k}})$ where $\overline{k}$ is the final iteration step index.
