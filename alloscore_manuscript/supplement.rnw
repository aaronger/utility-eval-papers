...
representing the future level of an undesirable outcome such as severe COVID incidence. Let $x \in \mathbb{R}$ be a decision variable
representing the level of some costly counter-measure, such as procurement of monoclonal antibody treatments,
that can be taken at a cost $C>0$ per unit in preparation for $Y$.  A decision maker must decide on a level $x$ of investment in the
...
shortage by a constant $L>C$ (which assumes that it is possible to quantify costs $C$ per unit of resource and losses $L$ per unit of shortfall on the same scale). We then combine the total shortage loss with expenditure into a \emph{loss function}\footnote
...
l(x,y) = Cx + L(y-x)_{+}.
\] 
The decision problem is then to select an action $x$ in a way that aligns with the preference that the random future loss $l(x,Y)$ be as low as possible.
...
\end{align}
is defined using interval addition $[a,b] + [c,d] = [a+c, b+d]$.
...
Otherwise, by evaluating $TQ_F$ at $\tau_M = \frac{1}{2}\left(\tau_L + \tau_U\right)$ we can replace $I_1$ with the interval
\begin{align}
I_2 = 
\begin{cases}
...
to $\tau^{\star}$ from below and above. 

In the generic case of an infinite $\{I_k\}$, we need to define practical stopping criteria for each of three possible behaviours of 
$F_T$ at $K$:
\begin{samepage}
\begin{itemize}
\item[($\,\mathrlap{\,\bullet}{\diagup}\,$)] $\tau^{\star} = F_T(K-)$ and $TQ_F(\tau^{\star}) = \{K\}$ (that is, if $F_T$ is continuous and strictly increasing at $K$)
\item[(\raisebox{-.5ex}{$-$}\!\!\raisebox{.5ex}{$\bullet\!-$})] $F_T$ has a point mass at $K$
\item[($-\!\!\!\bullet\!\!\!-$)] $K \in (q^{-}_{F_T,\tau^{\star}}, q^{+}_{F_T,\tau^{\star}})$ (that is, all component forecasts $F_i$ have zero density at points $x_i \in (q^{-}_{F_i,\tau^{\star}}, q^{+}_{F_i,\tau^{\star}})$ and $\sum x_i = K$).
\end{itemize}
\end{samepage}
