...
a solution $\tau^{\star}$ which offers control (at least in principle) of the Bayes risk error or ``optimality gap''
\begin{align}
...
in essence, that a forecaster cannot perceive any forecast at odds with their beliefs about the events being forecasted as having an optimal expected score.
We formalize this in terms of forecaster ``self-assessment'' in section \ref{sec:proper_def} and then address the propriety of the allocation score in three stages:
\begin{itemize}
...
We consider a setting where a forecasting exercise (such as a forecast hub) pre-specifies that forecasts will be represented using a parametric family of forecast distributions $F_\theta(y)$, and the task of the forecaster is to select a particular parameter value $\theta$. We use $\mathcal{P}$ to denote the collection of all distributions $F_\theta$ in the given parametric family. For instance, it has recently been proposed that mixture distributions could be used to represent forecast distributions \cite{wadsworth2023mixture}. Additionally, we note that the functionality in \verb`distfromq` can be viewed as specifying a parametric family $\mathcal{P}_{\mathrm{dfq}}$ where the parameters $\theta$ of $F_\theta$
are its quantiles at pre-specified probability levels, and where the shape of any $F_\theta \in \mathcal{P}_{\mathrm{dfq}}$ over the full range of its support is entirely controlled by these quantiles.
...
\apg{I think we need to say something about what we mean by ``close''.  The score divergence is what we are confronting here and I appreciate that 
others may not want to broach the topic.  But I think we have to say something about ``score expectation defined distance''. This also relates directly to the notation crunch below.}
