...
a solution $\tau^{\star}$ which offers control (at least in principle) of the Bayes risk error
\begin{align}
\left|\mathbb{E}_F[s_A(\bm{q}_{\tilde{\tau},F}, Y)] - \mathbb{E}_F[s_A(x^{F,K}, Y)]\right|
\end{align}
...
A desirable feature of a scoring rule is that it is \emph{proper}, which roughly means that it rewards a forecaster for stating their true beliefs about the events being forecasted. We give a more formal definition of propriety in section \ref{sec:proper_def} before addressing the propriety of the allocation score in three stages:
\begin{itemize}
  ...
  \item Section \ref{sec:distfromq_alloscore_proper_prospective} shows that the allocation score is still proper if the forecast is represented as a member of a parametric family of distributions, so long as this representation and scoring procedure was specified prospectively by the hub. This includes the use of methods such as \verb`distfromq` for constructing forecast distributions as a special case.
  \item Section \ref{sec:distfromq_alloscore_improper_post_hoc} shows that \emph{post hoc} application of this scoring procedure to quantile forecasts is improper, and makes some suggestions for organized forecasting efforts who are considering using the allocation score.
\end{itemize}
...
By construction, this procedure yields a proper score since
\begin{align}
\Ex_F [S(F, Y)] &= \Ex_F [ s(x^F, Y) ] \nonumber \\
 &\leq \Ex_F [ s(x^G, Y) ] \label{eqn:dt_proper_key} \\
 &= \Ex_F [ S(G, Y)]. \nonumber
\end{align}
Here, step~\eqref{eqn:dt_proper_key} holds because the Bayes act $x^F$ was selected so that the expected loss $\Ex_F [ s(x^F, Y) ]$ for that action is less than or equal to the expected loss $\Ex_F [ s(x, Y) ]$ for any other action $x$ (and for the action $x^G$ in particular).

Because the alloscore was developed using this procedure, it is a proper score.
...
The second is the forecaster's reporting problem where the task is to select parameter values $\theta$ to report. The forecaster's loss is given by $s_R(\theta, y) = s_A(x^{F_\theta}, y)$, where $x^{F_\theta}$ is the Bayes act for the allocation problem under the distribution $F_\theta$. In words, the loss associated with reporting $\theta$ is equal to the loss associated with taking the Bayes allocation corresponding to the distribution $F_\theta$.

Following our usual construction, the Bayes act for the forecast reporting problem is the parameter set that minimizes the forecaster's expected loss:
\begin{align*}
\theta^F &= \text{argmin}_\theta \Ex_F [s_R(\theta, Y)] \\
&= \text{argmin}_\theta \Ex_F [s_A(x^{F_\theta}, Y)]
\end{align*}
This leads the forecaster to choose values of the parameters $\theta$ such that the allocation $x^{F_\theta}$ is as close as possible to the allocation $x^F$ under the forecaster's true forecast distribution $F$.

We then arrive at the scoring rule
$$S_R(F, y) = s_R(\theta^F, y) = s_A(x^{F_{\theta^F}}, y).$$
\elr{Notation got pretty bad here... borderline about whether it's worth searching for a better notation for "the bayes allocation for the distribution with the bayes parameters"}
It follows from the discussion in section \ref{sec:alloscore_proper} that this is a proper scoring rule for $F$. Additionally, we note that the score can be calculated from the reported parameter values $\theta^F$.
...
It is, however, necessary to specify the parametric family to use and the foundational scoring rule $s_A$ (including any relevant problem parameters such as the resource constraint $K$) in advance, so that forecasters can identify the Bayes act parameter set $theta^F$ to report.

As an alternative, a forecast hub could ask forecasters to provide the Bayes allocations associated with their forecasts for one or more fixed resource constraints $K$, allowing for direct calculation of scores and narrowing the gap between model outputs and public health end users.

\subsection{Post hoc evaluation of quantile forecasts using distfromq and the allocation score is improper}
\label{sec:distfromq_alloscore_improper_post_hoc}

In this section, we illustrate that a post hoc evaluation of quantile forecasts using \verb`distfromq` and the allocation score is not proper. In brief, this is because the Bayes parameters $\theta^F$ for the forecaster to report may not align with quantiles of the distribution $F$ at the specified probability levels. We illustrate this with examples (borrow from elicitation.R...)
