...
solution $\bm{q}_{\tau^{\star},F} \in \undertilde{\bm{F}}^{-1}(\tau^{\star})$ to \eqref{eqn:quantiles-sum-to-K} where
$\tau^{\star}$ solves \eqref{eqn:tau-inclusion}. For practical scenarios in which the forecast $F$ has a form preventing 
analytical solutions to \eqref{eqn:quantiles-sum-to-K}, we must develop a method to approximate solutions to \eqref{eqn:tau-inclusion}
which lead to adequate approximations of the linear program
\begin{align}
