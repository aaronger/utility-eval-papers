...
\begin{align*}
    S_{CRPS}(F, y) &= \int S_Q(F, y; C, L) h(C, L) \, dC dL
\end{align*}
The usual CRPS results from taking a distribution $h$ such that the induced distribution on the probability level $\alpha = 1 - C/L$ is $\text{Uniform}(0,1)$.
The weighted interval score (WIS) can be used when the full forecast distribution $F$ is not available, and amounts to an approximation of CRPS where the distribution $h$ is discrete with point masses at the quantile levels corresponding to the endpoints of a finite set of prediction intervals.

% The below was an unsuccessful attempt to connect to a more usual notation for CRPS just in terms of integrating across quantile levels. It resulted in a notational quagmire, but I'm keeping it here for now in case it seems like a better way to go...
%
%To make the connection to the usual notation, we can divide the loss function $s_Q$ by $L$ to obtain an expression in terms of only $\alpha$.
%\begin{align*}
%s_Q(x,y; \alpha) &= s_Q(x,y; C, L)/L \\
%&= (C/L)x + (x-y)_- \\
%&= (1 - \alpha)x + (x-y)_-
%\end{align*}
%Because these loss functions are equal up to a constant of proportionality, the Bayes act is the same for both. We can how obtain a weighted CRPS by integrating the quantile score across values of $C$ and $L$ or across values of $\alpha$:
%\begin{align*}
%S_{CRPS}(F, y) &= \int S_Q(F, y; C, L) h(C, L) \, dC dL %\\
%&= \int S_Q(F, y; \alpha) h(\alpha) \, d\alpha
%\end{align*}
%The usual CRPS results from taking $h$ such that the distribution on the probability level $\alpha$ is $\text{Uniform}(0,1)$.
