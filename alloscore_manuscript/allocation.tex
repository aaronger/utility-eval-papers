...
In this work, we hope to begin to fill this gap between the ways in which infectious disease forecasts have been used to support public health policy and the ways in which they have traditionally been evaluated.
For concreteness, we consider a motivating application in which forecasts of public health surveillance signals are used to help determine the allocation of a limited quantity of medical supplies across multiple regions.
We define a new forecast scoring rule --- the {\em allocation score} --- that explicitly evaluates forecasts based on how beneficial their implied allocations would turn out to be.
Unlike the traditional measures of forecast skill such as WIS, CRPS, and log score, the proposed allocation score is rooted in this decision making context and measures the suitability of a forecast as an input to that specific decision making task.

Briefly, the allocation score of a forecast is the unnecessary unmet demand that results from using that forecast to set resource allocations by minimizing expected unmet need.
...
If there is a limited amount $K$ of the medical resource that is available to distribute, a decision-maker could choose an allocation of that resource across locations that minimizes the expected unmet need according to that forecast.
...
\elr{I have abandoned the attempt to get a succinct one-sentence definition of the allocation score at the beginning here, because it felt like we were trying to pack too many ideas into one sentence. But I'm open to revisiting. Maybe the definition of alloscore can be moved earlier?}
...
If the decision-maker is provided with a multivariate forecast $F$ that predicts resource demand in each of the $n$ locations, one possibility is that they may choose the resource allocation that minimizes the expected total unmet demand according to the forecast.
...
This stochastic allocation problem has an intuitively appealing solution: allocate so that the probabilities of need exceeding allocation in various locations are as close to each other as possible (see methods).
...
After time passes, the quality of a selected allocation can be measured by comparing the actual observed demand in each location to the amount of resources that were sent there. Specifically, we compute the total unmet demand that resulted from the selected allocation:
\begin{align}
...
The \textbf{allocation score} of the forecast $F$ is the unnecessary unmet demand that results from using the allocation that minimizes the expected unmet need according to that forecast.
By ``unnecessary'' we mean the unmet need that could have been avoided by an oracle that knows exactly how much need will occur in each location and divides the amount $K$ so that nothing is wasted in one location while it could be put to use in another.

\paragraph{Example 1} Suppose we have a forecast $F$ for demand in two locations with $F_1 = \mathrm{Exp}(1 / \sigma_1)$ and $F_2 = \mathrm{Exp}(1 / \sigma_2)$, with $\sigma_1 = 2$ and $\sigma_2 = 3$. If we have $K = 5$ units of our resource available, the optimal allocation according to $F$ would be 2 units in location 1 and 3 units in location 2. If, on the other hand, we have $K = 10$ units available, we will allocate 4 units of resources in location 1 and 6 units in location 2. Figure~\ref{fig:exp_alloc_example} illustrates the situation, and derivations of the optimal allocations are given in section *** of the supplemental materials.

\begin{figure}
    \includegraphics[width=\textwidth]{../figures/exponential_pred_expected_loss.pdf}
    \caption{An illustration of the resource allocation problem in Example 1. There are $n = 2$ locations, with predictive distributions $F_1 = \mathrm{Exp}(1/2)$ and $F_2 = \mathrm{Exp}(1/3)$, illustrated in the panels at bottom and right. In the center panel, the background shading corresponds to the expected loss according to these forecasts. Diagonal black lines indicate resource constraints at $K=5$ and $K=10$ units. For these forecasts, the optimal allocations are $(2,3)$ for $K=5$ and $(4,6)$ at $K=10$. These allocations are at the point on the constraint line where the expected loss is smallest, which also corresponds to the point where a level set of the expected loss surface (blue curve) is tangent to the constraint.}
    \label{fig:exp_alloc_example}
\end{figure}

Next suppose that we observe needs of 2 and 9 in locations 1 and 2, respectively. With $K = 10$ units of the resource, the oracle would be able to prevent all
but 1 of the total 11 units of needs from going unmet, for example by allocating 2 units of resources to location 1 and the remaining 8 units of resources to location 2.
However, the forecast $F$'s allocation results in $9 - 6 = 3$ units of unmet need in location 2 despite leaving no need unmet in location 1.
The allocation score for the forecast when $K = 10$ would therefore be 2 (= 3 realized $-$ 1 unavoidable) in units of unnecessary unmet need.
When $K = 5$, however, the forecast does not ``over-allocate'' in either location which leads to the same unmet need of $11 - 5 = 8$ as does the oracle's allocations.
The allocation score for $F$ when $K = 5$ is therefore 0.

Note the way in which $F$ incurred a positive (i.e., non-optimal) allocation score of 2 when $K = 10$. It was not directly directly due to individual misalignments 
...
Restating: as far as allocation decisions are concerned, with a fixed constraint $K=10$ the fundamental problem with the forecast $F$ is not that it predicts a mean total resource demand of $5$ units; it is that the realized demand was 4.5 times as large in location 2 as in location 1, but the forecast only indicated that the resource allocation for location 2 should be 1.5 times the allocation to location 1.
...
This stands in marked constrast to other common scoring methods for multi-variate forecasts that aggegate univariate scores such as CRPS or WIS for the marginal forecasts where a 
misalignment in one coordinate is penalized regardless of alignments in other coordinates. \apg{Should this be illustrated here, or elsewhere?} \elr{Not sure what I think. Comparisons with WIS/etc seem critical, but some of the text I've added here feels like text for the discussion rather than up here in the ``methods"...}
Note that we do not claim the allocation score is generically preferable to these other scores \textemdash rather, it provides a view of forecast performance that is specifically tuned to the context of decision-making about resource allocations.
