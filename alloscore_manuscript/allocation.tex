...
\subsection{A decision theoretic derivation of quantile score and allocation score}
\label{sec:methods.quantileloss}

In this section, we demonstrate that the quantile score and the allocation score can both be derived from the procedure outlined in section \ref{sec:methods.decisiontheory} based on two different, but closely related, decision-making problems. The results for quantile score are well-known in the literature (cite cite cite).

\paragraph{Problem definitions and loss functions.}
In both decision-making problems, the decision-maker must determine the level $x_i$ of a resource, such as a medical supply, that will be available to people who may need it at some future time in locations indexed by $i$. We use the variable $y_i$ to denote the eventual realized need for the resource in that location, e.g. the number of patients in need of a hospital bed or the amount of medication that is needed. Each unit of unmet need incurs a loss $L$, so that if the selected resource level $x_i$ is less than the realized need $y_i$, a loss of $L(y_i - x_i)$ results. At the time that a decision must be made, the demand $y_i$ is not yet known. We therefore define the random variable $Y_i$ that represents the as-yet-unknown level of demand in location $i$. The forecast $F_i$ specifies a predictive distribution for $Y_i$. Here we identify $F_i$ with its cumulative distribution function (CDF), and $F_i^{-1}$ denotes the quantile function. We use the notation $x = (x_1, \ldots, x_n)$, $y = (y_1, \ldots, y_n)$, $Y = (Y_1, \ldots, Y_n)$, and $F = (F_1, \ldots, F_n)$ to collect resource levels, resource demand, and forecasts across all locations.
...
%In both decision-making problems, the decision-maker is required to select a course of action that is designed to meet the future demand for a resource, which is unknown at the time that a decision must be made. The primary difference between these problems is the type of actions available to the decision-maker and the challenge (cost or resource constraint). TODO: clarify goal, minimize total costs.

The differences between the two decision-making problems primarily lie in what actions are available to the decision-maker and what trade-offs they must consider between different alternatives.
In decision problem 1, which will lead to the quantile score, the challenge faced by the decision-maker is that each unit of the resource has cost $C$, so that the total cost of procurement for location $i$ is $C x_i$. Summing across locations and combining the costs of procurement with the losses associated with procuring fewer resources than are needed, we arrive at the following loss function (subscripted by $Q$ for \emph{q}uantile):
\begin{equation}
s_Q(x, y; C, L) = \sum_{i=1}^n C x_i + L(y_i - x_i)_+. \label{eqn:quantile_loss_fn}
\end{equation}
The decision-maker's goal is to minimize this loss, but there is no hard limit on the available resources or the amount of funds that can be spent to purchase resources.
It follows that although we have written a single loss function that sums across all locations, this loss function is separable and in practice decisions about the quantity of resources to procure can be made separately for each location.

In decision problem 2, which will lead to the allocation score, a fixed quantity $K$ of the resource is available, and the problem is how best to allocate those resources to different locations. Thus, unlike in problem 1, here we must consider all locations simultaneously and any valid decision is required to satisfy the constraint $\sum_i x_i \leq K$. On the other hand, since the total amount of available resources is fixed, it is not relevant to consider the cost of procuring resources. When the resource constraint is satisfied, the overall loss function (subscripted by $A$ for \emph{a}llocation) is:
\begin{equation}
s_A(\mathbf{x}, \mathbf{y}; L) = \sum_{i=1}^n L (y_i - x_i)_+. \label{eqn:alloc_loss_fn}
\end{equation}

\paragraph{Bayes acts.}
Now that we have formalized the decision-making tasks with loss functions that measure the quality of a selected action $x$ in relation to realized demand levels $y$, we can identify the Bayes acts corresponding to these loss functions. It can be shown that under each of these loss functions, the Bayes act is a quantile of the forecast distribution in each location (see supplemental section *** for the derivations):
\begin{align}
    x^{F}_i = F_i^{-1}(\alpha).
\end{align}    
The probability level $\alpha$ for this quantile is the same across all locations, but it is different for the Bayes acts derived from the quantile loss and the allocation loss.

Under the quantile loss, the Bayes act sets resource levels to be quantiles of the forecast distributions at the probability level $\alpha_Q = 1 - C/L$.
\elr{Not confident in the following sentences' correctness, or in their value in this manuscript. We could just try to gloss over all of this or summarize with a less precise statement about the cost-loss ratio? $C/L > 1$ is also a possibility that it's awkward to address...}
With this choice, the probability that demand exceeds the selected resource supply is equal to $C/L$. It does not make sense to purchase more resources than this because the cost $C$ of an additional unit of resources would be greater than $L$ times the forecasted probability of an additional unit of resource demand. In the edge case where the cost $C$ of purchasing resources is greater than the loss $L$ associated with under-purchasing resources, so that $\alpha_Q < 0$, the decision-maker will not purchase any resources.

Under the allocation loss, the Bayes act chooses quantiles at a probability level that depends on the resource constraint $K$, the loss $L$, and the forecast distributions, and is selected so that the allocations do not exceed the resource constraint:
\begin{align}
\alpha_A &= 1 - \lambda^{\star}/L \text{, where $\lambda^\star$ is selected so that} \\
\sum_{i=1}^{N}F_i^{-1}(1 - \lambda^{\star}/L) &= K.
\end{align}
This partial solution to the allocation problem seems to have first appeared in \cite{hadleywhitin1963}.
One interpretation of this result is that the Bayes act divides the available resources among the locations in such a way that the forecasted probability that resource demand will exceed the available resources is the same in all locations.
...
\paragraph{Scoring rules.} We can now define scoring rules for a forecast $F$ by evaluating the loss function at the Bayes act corresponding to that loss function. This leads to the following scoring rule for quantile forecasts:
\begin{align*}
S_Q(F, y; C, L) &= \sum_{i=1}^n s_Q(x^F, y; C, L) = \sum_{i=1}^n Cx_i^F + L(y_i - x_i^F)_+ \nonumber \\
&= C F_i^{-1}(\alpha_Q) + L \left(y_i - F_i^{-1}(\alpha_Q)\right)_+ \label{eqn:S_Q_CL}
\end{align*}
This score measures the total cost of resource purchases and losses incurred due to unmet need that results from using the Bayes resource procurements associated with the forecast $F$ when the actual level of need in each location is observed to be $y_i$.

On the other hand, the allocation scoring rule is:
\begin{equation}
S_A(F, y; L, K) = s_A(\mathbf{x}^F, y; L) = \sum_{i=1}^n L (y_i - F_i^{-1}(1 - \lambda^{\star}/L))_+
\end{equation}
This score measures the total unmet need across all locations that results from using the Bayes allocation associated with the forecast $F$ when the observed need is $y_i$.

We have defined the quantile scoring rule $S_Q$ in terms of the cost and loss parameters to connect to the decision-making context. 
To connect to the usual notation for the quantile score, we can divide through by $L$ to obtain an expression in terms of only $\alpha$:
% \begin{align*}
...
\elr{It's a little tempting to just divide by the loss parameter $L$ from the start and simplify throughout?}

\subsubsection{Integrated scores}
\label{sec:methods.quantileloss.crps}

\elr{Not updated yet, but in this subsection the plan would be to talk about CRPS as integrated quantile score, integrated allocation score, and connections between them.}
...
\paragraph{Step 2: Given a probabilistic forecast $F$, identify the Bayes act.} 
\paragraph{Step 3: Define the scoring rule.} 
