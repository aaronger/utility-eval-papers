...
Briefly, the allocation score of a forecast is the unnecessary unmet demand that results from using that forecast to set resource allocations by minimizing expected unmet need. In a little more detail, the total expected unmet need according to the forecast is
\begin{align}
\sum_i E_{F_i}[\text{unmet need in location $i$}],
\end{align}
where the $F_i$ are the marginal forecast distributions of demand in each location; for example, these might be forecasts of the level of need for medical resources in each state or hospital system.
If there is a limited amount $K$ of a resource that is available to distribute, a decision-maker could choose an allocation of that resource across locations that minimizes this expected unmet need.
...
According to the allocation score, one forecast is better than another if it would lead decision-makers to an allocation that results in less unmet need. We elaborate on these ideas in Section~\ref{sec:methods}.
...
The allocation score of a multivariate forecast $F$ is the unnecessary realized unmet demand when $F$ is used to allocate so as to minimize expected unmet 
...
\end{align}
is minimized, where the $F_i$ are the marginal distributions of $F$. By ``unnecessary'' we mean the unmet need that could have been avoided by an oracle that knows exactly how much need will occur in each location and divides the amount $K$ so that nothing is wasted in one location while it could be put to use in another.
