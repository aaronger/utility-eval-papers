...
\begin{align}
    \mathbf{x}^F &= \mathrm{arg min}_{\mathbf{x}}\,\, \mathbb{E}_{F} [s_A(\mathbf{x}, \mathbf{Y}; L)] \text{ subject to }
    0 \leq x_i \text{ for all $i$ and} \, \sum_{i=1}^N x_i = K, \label{AP}
\end{align}
where $\mathbb{E}_{F} [s_A(\mathbf{x}, \mathbf{Y}; L)] = \sum_{i=1}^{N} L \mathbb{E}_{F_i}[(Y_i - x_i)_+]$ sums the expected loss due to unmet need across all locations. It can be shown that the Bayes act has $x^F_i = F_i^{-1}(1 - \lambda^{\star}/L)$, where $\lambda^{\star}$ depends on the problem parameters $K$ and $L$ as well as the forecast distributions and is chosen so as to satisfy the equation
\begin{align}
...
An alternative interpretation comes from noting that for each location $i$, $\frac{\partial}{\partial x_i} \mathbb{E}_{F} [s_A(\mathbf{x}, \mathbf{Y}; L)] \vert_{\mathbf{x}=\mathbf{x}^F} = - \lambda^{\star}$ (see the supplement for a proof).
