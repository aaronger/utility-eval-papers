...
- started as RA with Nick and Evan at [reichlab.io](reichlab.io) at UMass in 2021
...
**Continuous Ranked Probability Score** (a probabilistic generalization of MAE)
...
by which a DM will minimize **expected loss** $E_Y[l]$ 
assuming that in reality $Y \sim F$.
...
Key point: Given $l$, we have an *automatically proper* scoring rule 
$S(F,y) := l(x^F, y)$
...
So we resort to a binary search for $\lambda^{\star}$ following long history in inventory management of solving **constrained multi-product newsvendor** 
problems^[See e.g., Zhang, Bin, Xiaoyan Xu, and Zhongsheng Hua. 2009. “A Binary Solution Method for the Multi-Product
Newsboy Problem with Budget Constraint.”]
...
- ranking at peak seems to be consistent with that in large $K$
region.
...
We hope/think that allocation scoring is sensitive to implicit dependence structures in forecasts, but all work so far only refers directly to marginals
- nothing yet with copulas, etc.
...
[https://github.com/aaronger/alloscore](https://github.com/aaronger/alloscore)

A less rough package Evan wrote to implement cdf reconstruction

[https://github.com/reichlab/distfromq](https://github.com/reichlab/distfromq)
...
An explicit vacabulary of social loss in ID forecast evaluation has instrinsic advantage
over "off-the-shelf" metrics from statistic inference
