...
If I've correctly translated from the notes document, the expected loss function is
\begin{equation*}
\bar{s}_F(\mathbf{x}) = \sum_{i=1}^2 \mathbb{1}\{0 \leq x_i\}\left[ \sigma e^{-x_i/\sigma} - \sigma \right]
\end{equation*}

I'm going with this below because I think it's close, but I don't actually think it's correct because it leads to negative loss values.
...
x_1 <- qexp(tau, rate = 1 / sigma_1)
x_2 <- qexp(tau, rate = 1 / sigma_2)
...
joint_dist <- y_grid %>%
    dplyr::mutate(
        s_bar = sigma_1 * exp(-y_1 / sigma_1) - sigma_1 +
                sigma_2 * exp(-y_2 / sigma_2) - sigma_2
    )
...
ggplot(data = joint_dist) +
    ...
    geom_point(x = x_1, y = x_2, color = "orange") +
