...
By making the forecaster aware of $s$ prior to prediction, we elicit an $\alpha$-level quantile $q_{\alpha,F}$ of the forecast distribution $F$ for $Y$. To see this, notice that if a forecaster believes that $Y \sim F$, they will calculate their expected score to be
\begin{align} 
\sbar_F(x) = E_F[s(x,Y)]  &= O E_F[(g(x) - g(Y))\sb{+}] + U E_F[(g(x) - g(Y))\sb{-}]. \label{eqn:scalar_obj}
\end{align}
In order to minimize this expectation, they must forecast a solution of the first order equation
\begin{align}
...
\begin{align}
\sbar_F(x) &= O \int_{\mathbb{R}} \int_{\mathbb{R}} \mathbb{1}\{Y \leq u < x\}dg(u) dF(y) + U \int_{\mathbb{R}} \int_{\mathbb{R}} \mathbb{1}\{x \leq u < Y\}dg(u) dF(y) \\
...
or
\begin{align}
\nonumber \sbar_F(x) &= O\int_{-\infty}^x(g(x) - g(y))dF(y) + U\int_x^{\infty}(g(y) - g(x))dF(y)\\ 
...
If, on the other hand, $F=F_{\mu, \sigma}$ lies in a location-scale family, that is, $F(x)= F_0\left(\frac{x-\mu}{\sigma}\right)$, we have
\begin{align}
...
\begin{align}
\sbar_F(x) &= \kappa\left[x(F(x) - \alpha) - \mu F(x) - \sigma \int_{-\infty}^{\frac{x-\mu}{\sigma}}zdF_0(z)  + \alpha E_F[Y]\right] \\
...
Thus, outside $(a,b)$, $\sbar_{U[a.b]}(x)$ has the form of pinball loss centered on the mean $m_F = \frac{a+b}{2}$ and is interpolated on $(a,b)$ 
 by a quadratic centered at the quantile $a+\alpha(b-a)$ with slopes matching the linear tails at $a$ and $b$ (see figure 1).

Or if $F=\Phi_{\mu,\sigma}$ is normal with location $\mu = E_F[Y]$ and scale $\sigma = \sqrt{\mathrm{Var}(Y)}$, 
we have 
\begin{align}
...
so that
\begin{align}
\sbar_F(x) = \kappa\left[(\Phi_{\mu,\sigma}(x) - \alpha)(x-\mu) + \sigma^2 \varphi_{\mu, \sigma}(x)\right].
\end{align}
...
\begin{align}
\sbar_F(x) &=  O \int_{-\infty}^{g(x)} F(g^{-1}(u)) du + U \int_{g(x)}^{\infty}(1-F(g^{-1}(u))) du
\end{align}
...
\end{align}
Note, for reference, that in the literature $\sbar_{n,F}(x)$ is often encountered in the form
\begin{align}
