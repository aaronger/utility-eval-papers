...
For a general scoring rule $S$, the $S$-entropy of $F$ is defined as 
\begin{align}
...
\end{align}
If a forecaster believes that $Y \sim F$ then they expect to receive a score of $e_{\alpha}(F)$ by reporting their $\alpha$ quantile.
...
&= \alpha \overline{g}\sb{F}-\overline{g}_{F}(x).
\end{align}
...
\end{align}
To emphasize the reality of $G$ we write $e_{\alpha}(G)$ as $e_{\alpha}(Y)$, which with \eqref{egn:divergence}
gives 
\begin{align}
S_{\alpha}(F,G) = d_{\alpha}(F,G) + e_{\alpha}(Y). \label{eqn:marg_score_decomp}
\end{align}
This expresses the true expected score of $F$ 
...
and $r(y-x)\sb{+}$ of monetarily valued customer "goodwill" is lost. 
...
\begin{align}
\sbar_{F, {\mathbf{n}}}(x) = (c-u)E_F[(x-Y)\sb{+}] + (p-c+r)E_F[(Y-x)\sb{+}] -(p-c)E[Y].
\end{align}
...
\begin{align}
Q = F^{-1}\left(\frac{U}{O+U}\right) = F^{-1}\left(\frac{p-c+r}{p-u + r}\right).
\end{align} 
Note, for reference, that in the literature $\sbar_{F, {\mathbf{n}}}(x)$ is often encountered in the form
\begin{align}
&(O+U) \int_{0}^{x} (x - y)dF(y) - Ux+ (U-u+r)E[Y] \\{}
=\,&(p+r-u) \int_{0}^{x} (x - y)dF(y) - (p-c+r)x+ rE[Y]
\end{align}
