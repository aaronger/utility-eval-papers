...
\begin{align}
s(x,y)=s_{O,U}(x,y) &= O(g(x) - g(y))\sb{+} + U(g(y)-g(x))\sb{+} \label{eqn:scalar_score}
\end{align}
...
By making the forecaster aware of $s$ prior to prediction, we elicit an $\alpha$-level quantile $q_{\alpha,F}$ of the forecast distribution $F$ for $Y$, 
where $\alpha = U/(U+O)$. To see this, notice that if a forecaster believes that $Y \sim F$, they will calculate their expected score to be
\begin{align} 
\sbar_F(x) = E_F[s(x,Y)]  &= O E_F[(g(x) - g(Y))\sb{+}] + U E_F[(g(Y) - g(x))\sb{+}]. \label{eqn:scalar_obj}
\end{align}
...
Note from \eqref{eqn:scalar_score_ident} that $\frac{d}{dx}\sbar_F(x) =\frac{d}{dx}E_F[s(x,Y)] = k(x)E_F[V(x,Y)]$, where $k(x) = \kappa g^{\prime}(x)$ and 
\begin{align}
V(x,y) 
...
\end{align}
The estimand here is the value at $G$ of the functional $\mathcal{\overline{S}}$ on $\mathcal{F_0}$ given by
\begin{align}
\mathcal{\overline{S}}_{G,\alpha}(F) := \sbar_G(Q_{\alpha}(F))
\end{align}
where $Q_{\alpha}$ is the $\alpha$ quantile functional. This is the expected value under $s$ of the forecaster's $\alpha$ quantile under the \emph{actual} data generating process $G$.
...
\subsection{Alternative Notation Possibilities}
...
\begin{align}
s_n(x,y) & = a_x x +a_y y +a_{+}(x-y)\sb{+} + a_{-}(y-x)\sb{+}.
\end{align}
We might have, for example, the score given by net value transfers
\begin{align}
s_n(x,y) = cx-py - u(x-y)\sb{+} + (p+r)(y-x)\sb{+}
\end{align}
where $x$ newspapers are ordered at price $c$, $y - (y-x)\sb{+}$ are sold at retail price $p$, $(x-y)\sb{+}$ are sold at salvage price $u$, 
and $r(y-x)\sb{+}$ of customer "goodwill" is lost. 
This can be rewritten as
\begin{align}
s_n(x,y) &= (a_{+} + a_x)(x-y)\sb{+} + (a_{-} - a_x)(y-x)\sb{+} +(a_x + a_y)y \\
&= (c-u)(x-y)\sb{+} + (p+r-c)(y-x)\sb{+} -(p-c)y
\end{align}
giving the retailer's objective function under a demand distribution $F$ as
\begin{align}
\sbar_{n,F}(x) = (a_{+} + a_x)E_F[(x-Y)\sb{+}] + (a_{-} - a_x)E_F[(Y-x)\sb{+}] +(a_x + a_y)E[Y].
\end{align}
