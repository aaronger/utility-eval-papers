...
\item $F$ denote the forecast distribution for the level of future need $Y$
\item $Y \sim G$
...
We measure the forecaster's performance when the realized need is $y$ with the scoring function
\begin{align}
s(x,y)=s_{O,U}(x,y) &= O(g(x) - g(y))\sb{+} + U(g(x)-g(y))\sb{-}. \label{eqn:scalar_score} \\
&= \kappa((1-\alpha)(g(x) - g(y))\sb{+} + \alpha(g(x)-g(y))\sb{-}) \label{eqn:scalar_score_alpha} \\
&= \kappa(\mathbb{1}\{x>y\} - \alpha)(g(x)-g(y)) \label{eqn:scalar_score_ident}\\
&:= s_{\kappa, \alpha}(x,y) := \kappa s_{\alpha}(x,y),
\end{align}
where $\alpha = U/(U+O)$, $\kappa = U+O$,
$u_+ := \max(u,0)$, $u_- := \max(-u,0) = (-u)_+$, and $\mathbb{1}\{A\}$ is the indicator function of the event $A$.
This scoring function $s$ is negatively oriented, i.e., a smaller score indicates better performance corresponding to a smaller cost due to under- or over-prediction.

By making the forecaster aware of $s$ prior to prediction, we elicit an $\alpha$-level quantile $q_{\alpha,F}$ of the forecast distribution $F$ for $Y$. That is, if a forecaster believes that $Y \sim F$, then according to this belief they minimize their expected score
\begin{align} 
\sbar_F(x) = E_F[s(x,Y)]  &= O E_F[(g(x) - g(Y))\sb{+}] + U E_F[(g(x) - g(Y))\sb{-}] \label{eqn:scalar_obj}\\
&= O\int_{-\infty}^x(g(x) - g(y))f(y)dy + U\int_x^{\infty}(g(y) - g(x))f(y)dy \label{eqn:scalar_obj_int}
\end{align}
by forecasting a quantile $q_{\alpha, F} \in Q_F\left(\alpha\right)$ which solves the first order equation
\begin{align}
\nonumber 0 =\frac{d}{dx}\sbar_F(x) 
&= O E_F\left[\frac{d}{dx}(g(x) - g(Y))\sb{+}\right] + U E_F\left[\frac{d}{dx}(g(x) - g(Y))\sb{-}\right] \\
...
&= \kappa g^{\prime}(x)\left(F(x) - \alpha\right) \label{scalar_Z_deriv}.
\end{align}
(See \hyperref[scalar_Z_deriv_detail]{appendix} for an alternative derivation in terms of a density.)
The critical point $q_{\alpha, F}$ is actually a minimum since \eqref{scalar_Z_deriv} is non-positive on $\{x<q_{\alpha, F}\}$ and non-negative on $\{x>q_{\alpha, F}\}$.
 When $g^{\prime}(q)>0$, this minimum is unique since
\begin{align}
   \frac{d^2}{dx^2}\sbar_F(x)\big|_{x=q_{\alpha, F}} &= 
   \kappa\left(g^{\prime}(x)f(x) + g^{\prime \prime}(x)\left(F(x) - \alpha\right)\right)\big|_{x=q_{\alpha, F}} \label{scalar_Z_deriv2}\\
   &=\kappa(g^{\prime}(q_{\alpha, F})f(q_{\alpha, F}) + g^{\prime \prime}(q_{\alpha, F})\cdot 0) >0.
\end{align}

If $g^{\prime}(q_{\alpha, F})=0$ or does not exist, or $Q_F(\alpha)$ is not a singleton set, we'll need to adopt additional criteria for whether $q_{\alpha, F}$ is optimal. One possibility is to take $q_{\alpha, F}=\min \left\{x \mid F_X(x) \geq \alpha\right\}$, and if $g^{\prime}(q_{\alpha, F})=0$ we choose the prediction $x$ to be the infimum of the region around $q_{\alpha, F}$ where $g$ is flat. One example of such a $g$ is the \emph{power curve} of a wind turbine which becomes constant above a certain wind speed. Here $x$ would be a forecast of future wind speed $Y$ at time $t$ and $g(x)$ would be the power a turbine operator commits to supplying the electric grid at $t$. Taking $g$ to be constant below a certain point could also serve to express the futility or lack of meaning of forecasts below that point, such as when a forecast user is only able to sell a commodity for which demand can become negative, turning into supply.

Note that $\frac{d}{dx}\sbar_F(x) =\frac{d}{dx}E_F[s(x,Y)] = h(x)E_F[V(x,Y)]$, where $h(x) = \kappa g^{\prime}(x)$ and 
\begin{align}
V(x,y) &\coloneq (1-\alpha)\mathbb{1}\{y<x\} - \alpha \mathbb{1}\{y \geq x\} \mathrel{\phantom{=}}\\
&= (1-\alpha)\mathbb{1}\{y<x\} - \alpha \left(1 - \mathbb{1}\{y<x\}\right) \\
&= \mathbb{1}\{y<x\} - \alpha. 
\end{align}
By virtue of $E_F[V(Q_F(\alpha),Y)]=0$, $V$ is said to be an \emph{identification function} for the $\alpha$ quantile. The fact that, generally speaking, any elicitable functional (such as a quantile) has an identification function is known as \emph{Osband's principle}.

Forecasting the minimizer of $\sbar_F(x) = E_F[s_{\kappa,\alpha}(x,Y)]$ in this situation is known as the \emph{Bayes act} $a_F$ under $s_\alpha$ for the forecaster. Assuming a forecaster is informed, rational, and risk-neutral enough to take $a_F$, we can evaluate $F$, implicitly, as a distributional forecast, by the \emph{scoring rule} $S_{\alpha}$ induced by $s_{\alpha}$:
\begin{align}
...
\end{align}
This defines a functional on $\mathcal{F_0}$, the class of measures on the Borel-Lebesgue sets of $\mathbb{R}$ by
\begin{align}
\mathcal{\overline{S}}_{G,\alpha}(F) := \sbar_G(Q_{\alpha}(F))
\end{align}
where $Q_{\alpha}$ is the $\alpha$ quantile functional.

Using $u_- = u_+ - u$, the objective function \eqref{eqn:scalar_obj} can also be helpfully rewritten as
\begin{align}
\nonumber \sbar_F(x) &= OE_F[(g(x) - g(Y))\sb{+}] + U E_F[(g(x) - g(Y))\sb{+}] - U E_F[(g(x) - g(Y))] \\ 
\nonumber  &= (O+U)E_F[(g(x) - g(Y))\sb{+}] - U E_F[(g(x) - g(Y))] \\
&= (O+U) \int_{-\infty}^{x} (g(x) - g(y))f(y)dy - Ug(x)+ U \overline{g}\sb{F} \label{obj_integral}\\
&= (O+U)\left(g(x)F(x) - \overline{g}_F(x) \right) - Ug(x) + U\overline{g}\sb{F}) \label{obj_sep}\\
&= \kappa\left[g(x)(F(x) - \alpha) -\overline{g}_F(x) + \alpha \overline{g}\sb{F}\right] \label{obj_pexp} 
\end{align}
where $\overline{g}_F(x) = E_F[g(Y)\mathbb{1}\{Y \leq x\}]$, and $\overline{g}\sb{F} = E_F[g(Y)]$.
...
##  Alternative Notation Possibilities

Let $Y \sim F,F_i$ be various forecaster beliefs and $Y \sim G$ be reality.
\begin{align}
...
Define the $s$-entropy of an arbitrary $F$ 
\begin{align}
e_s(F) &:= E_F[S(F, Y)] = E_F[s(q_{F,\alpha}, Y)] = s(q_{F,\alpha}, F) \\
\end{align}
and of the RV $Y$ as
\begin{align}
e_s(Y) &:= e_s(G).
\end{align}
Define the $s$-divergence between two forecasts as
\begin{align}
d_s(F_1,F_2) &:= S(F_1,F_2) - S(F_2,F_2) = S(F_1,F_2) - e_s(F_2)
\end{align}
and the $s$-divergence from $F$ to $Y$ as
\begin{align}
d_s(F,Y) &:= S(F,G) - e(Y).
\end{align}

Thus, $\overline{S}(F) = d_s(F,Y) + e_s(Y)$.

## Examples for various forecaster beliefs

### 1) $Y \sim U[a,b]$

Here $F(x) = \mathbb{1}\{a<x<b\}(x-a)/(b-a) + \mathbb{1}\{b \leq x\}$ in \eqref{obj_pexp} gives 
<!--  
&= 
\begin{cases}
\frac{U}{b-a}\int_a^b (g(y) - g(x))dy & x\leq a \\[8pt]
\frac{O}{b-a}\int_a^x (g(x) - g(y))dy + \frac{U}{b-a}\int_x^b (g(y) - g(x))dy & a < x < b \\[8pt]
\frac{O}{b-a}\int_a^b (g(x) - g(y))dy & b \leq x 
\end{cases} \\ 
-->
\begin{align}
\sbar_F(x) &= 
\begin{cases}
\frac{\kappa}{b-a}\left( - \alpha(b-a)g(x) + \alpha\int_a^b g(y)dy\right) & x\leq a \\[8pt]
\frac{\kappa}{b-a}\left((x-a- \alpha(b-a))g(x) - \int_a^x g(y)dy + \alpha\int_a^b g(y)dy\right) & a < x < b \\[8pt]
...
\sbar_F(x) &= 
\begin{cases}
\frac{\kappa}{b-a}\left( - \alpha(b-a)x + \frac{\alpha}{2}(b^2-a^2)\right) & x\leq a \\[8pt]
\frac{\kappa}{b-a}\left((x-a- \alpha(b-a))x - \frac{1}{2}(x^2 - a^2) + \frac{\alpha}{2}(b^2-a^2)\right) & a < x < b \\[8pt]
...
Thus $\sbar_F(x)$ has the form of pinball loss centered on the mean $\mu_Y = \frac{a+b}{2}$ and interpolated on $(a,b)$ 
 by a quadratic centered at the quantile $a+\alpha(b-a)$ with slopes matching the linear tails at $a$ and $b$.
...
That is, $Y = \mu + \sigma Z$ and $Z \sim F_Z = F_0$. Then $f_Y(y) = \dby{}{z} F_Z\left(\frac{y-\mu}{\sigma}\right)\dby{z}{y} = f_Z(z)/\sigma$ and so
\begin{align}
\overline{g}_F(x) = \int_a^x y f_Y(y)dy &= \int_{\frac{a-\mu}{\sigma}}^{\frac{x-\mu}{\sigma}}(\mu + \sigma z)f_Z(z)dz \\
& = \mu F_Z\left(\frac{x-\mu}{\sigma}\right) + \sigma \int_{\frac{a-\mu}{\sigma}}^{\frac{x-\mu}{\sigma}}zf_Z(z)dz\\
& =\mu F_Y(x) + \sigma \mu_{F_0}\left(\frac{x-\mu}{\sigma}\right).
\end{align}
where $\mu_{F_0}(u) = \int_{\frac{a-\mu}{\sigma}}^{u} zf_0(z)dz$. This gives
<!-- With the change of variables $y=Q_F(\alpha) = F^{-1}(\alpha), dy =  d(F^{-1}(\alpha)) = \frac{d\alpha}{f(y)}$, we have
\begin{align}
\overline{g}_F(x) = \int_a^x y f_Y(y)dy = \int_{0}^{F(x)}Q_F(\alpha)d\alpha 
& = \mu F(x) + \sigma \int_0^{F(x)} Q_0(\alpha)d\alpha \\
& =\mu F(x) + \sigma \mu_{F_0}\left(\frac{x-\mu}{\sigma}\right).
\end{align} -->
\begin{align}
\nonumber \sbar_F(x) &= \kappa\left[x(F(x) - \alpha) - \mu F(x) - 
\nonumber \sigma \mu_{F_0}\left(\frac{x-\mu}{\sigma}\right) + \alpha \mu \right]  \\
&= \kappa\left[\left(F(x) - \alpha\right)(x-\mu)- \sigma \mu_{F_0}\left(\frac{x-\mu}{\sigma}\right) \right]
\label{lc-form}
\end{align}
...
$$
Z_F(x) = \kappa\left[(\Phi_{\mu,\sigma}(x) - \alpha)(x-\mu) + \sigma^2 \varphi_{\mu, \sigma}(x)\right].
$$
...
Taking $\mu \equiv 0, Y = \sigma Z$ and $F_Z(x) = F_0(x) = \mathbb{1}\{0 \leq x\}(1 - e^{-x})$ we have
 $\mu_{F_0}(x) = \mathbb{1}\{0 \leq x\}(1 - e^{-x}(1+x))$. Then \eqref{lc-form} is

\begin{align}
\nonumber \sbar_F(x) &= 
\mathbb{1}\{0 \leq x\}\kappa\left[F(x)x - \sigma \mu_{F_0}\left(\frac{x}{\sigma}\right) \right] - \kappa\alpha x\\
&= \mathbb{1}\{0 \leq x\}\kappa
\left[
\left(1 - e^{-x/\sigma}\right)x - \sigma \left(1- e^{-x/\sigma}\left(1+\frac{x}{\sigma}\right) \right)
\right] - \kappa\alpha x\\
&= \mathbb{1}\{0 \leq x\}\kappa\left[ \sigma e^{-x/\sigma} + x - \sigma \right] - \kappa\alpha x
\end{align}

```{r echo = FALSE, cache=TRUE}
exp_loss <- function(alpha, rate) {
  f <- function(x){
    ((exp(-x*rate)*rate^(-1) + x - rate^(-1)))*(x>0) - alpha*x
  }
  return(f)
}
parms = list(rate = 1)
...
## Another form for $\sbar_F(x)$

There also may be situations where it is convenient to integrate the $(O+U)$-term in \eqref{obj_sep} by parts to get 
\begin{align}
\sbar_F(x) = \kappa\left[\int_a^x g^{\prime}(y)F(y)dy - \alpha(g(x) - \overline{g}\sb{F})\right]. \label{obj_intF}
\end{align}
...
\begin{align}
s_n(x,y) & = a_x x +a_y y +a_{+}(x-y)_+ + a_{-}(x-y)_-.
\end{align}
We might have, for example, the score given by net value transfers
\begin{align}
s_n(x,y) = cx-py - u(x-y)_+ + (p+r)(x-y)_-
\end{align}
where $x$ newspapers are ordered at price $c$, $y - (x-y)_-$ are sold at retail price $p$, $(x-y)_+$ are sold at salvage price $u$, 
and $r(x-y)_-$ of customer "goodwill" is lost. 
This can be rewritten as
\begin{align}
s_n(x,y) &= (a_{+} + a_x)(x-y)_+ + (a_{-} - a_x)(x-y)_- +(a_x + a_y)y \\
&= (c-u)(x-y)_+ + (p+r-c)(x-y)_- -(p-c)y
\end{align}
giving the retailer's objective function under a demand distribution $F$ as
\begin{align}
\sbar_{n,F}(x) = (a_{+} + a_x)E_F[(x-Y)_+] + (a_{-} - a_x)E_F[(x-Y)_-] +(a_x + a_y)E[Y].
\end{align}
This differs by a constant from our $\sbar_F(x)$ in \eqref{eqn:scalar_obj} after taking $g(x)=x$, $O=a_{+} + a_x=c-u$ 
and $U=a_{-} - a_=p+r-c$, and so has the same minimizer   
\begin{align}
Q = F^{-1}\left(\frac{U}{O+U}\right) =  F^{-1}\left(\frac{a_{-} - a_x}{a_{-} + a_{+}}\right) 
= F^{-1}\left(\frac{p+r-c}{p+r-u}\right).
\end{align}
Note, for reference, that in the literature $\sbar_{n,F}(x)$ is often encountered in the form of \eqref{obj_integral}:
\begin{align}
&(O+U) \int_{0}^{x} (x - y)f(y)dy - Ux+ (U+a_x+a_y)E[Y] \\
=\,&(p+r-u) \int_{0}^{x} (x - y)f(y)dy - (p+r-c)x+ rE[Y]
\end{align}
...
According to \eqref{eqn:scalar_obj_int} and \eqref{obj_pexp} the objective function is now
\begin{align}
...
As a separable problem, this is amenable to being solved via its dual (see [@ruszczynski2011nonlinear], section 4.4). With the Lagrangian
\begin{align}
...
\end{align}
which gives lower bounds on $\sbar_F(\mathbf{x}^{\star})$ at a solution $\mathbf{x}^{\star}$ to the primal AP. In particular, when $O_i>0$ for all $i$, $L_D(0)$ gives the total score $\sbar_F(\mathbf{q}_{\symbfit{\alpha}, F})$ when the constraint does not prevent the forecaster from giving the quantiles in each component. When the constraint is active, the forecaster will generally not be able to attain a total score this low and the greatest lower bound on their score will correspond the maximizer $\lambda^{\star} = \lambda^{\star}(K)$ of \eqref{dual_eq}.
...
s_{\symbfit{\beta}}(\mathbf{Q}^{\symbfit{\beta}}, \mathbf{y})&=\sum O_i(g_i(Q_i^{\symbfit{\beta}}) - g_i(y_i))_+ + U_i(g_i(Q_i^{\symbfit{\beta}})-g_i(y_i))_-
\\
...
\begin{align}
l(\mathbf{x}, \mathbf{y}) = U\sum_{i=1}^{N} (x_i - y_i)_-.
\end{align}
