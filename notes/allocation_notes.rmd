...
\begin{align}
e_{\alpha}(F) &:= S_{\alpha}(F, F) = E_F[s_{\alpha}(q_{\alpha,F}, Y)] .
\end{align}
A forecaster believing that $Y \sim F$ will expect to receiv`e a score of $e_{\alpha}(F)$ by reporting their $\alpha$ quantile, while an
omniscient forecaster \emph{knowing} that $Y\sim G$ will expect a score of $e_S(Y)$ for reporting $q_{\alpha,Y}$. Note that if 
$F_{y_0} = \mathbb{1}\{x \geq y_0\}$ is deterministic, then 
\begin{align}
e_{\alpha}(F_{y_0}) = E_{F_{y_0}}[s_{\alpha}(q_{\alpha, F_{y_0}}, Y)] = s_{\alpha}(y_0, y_0) = 0. \label{eqn:entropy_determ}
\end{align}
Similarly, if $F_{\mathbf{y}_n} = \mathrm{Unif}\{y_1, \ldots, y_n\}$ is the empirical distribution of a sample $\mathbf{y}_n = \{y_1, \ldots, y_n\}$, then
\begin{align}
e_{\alpha}(F_{\mathbf{y}_n}) = E_{F_{\mathbf{y}_n}}[s_{\alpha}(q_{\alpha, F_{\mathbf{y}_n}}, Y)] 
& = \frac{1}{n}\sum_{i=1}^{n} s_{\alpha}(y_{\lceil \alpha n \rceil}, y_i) \\
& = \frac{1}{n} \left\{(1-\alpha)\sum_{i=\lceil \alpha n \rceil}^{n}g(y_i) - g(y_{\lceil \alpha n \rceil}) + 
\alpha \sum_{i=1}^{\lceil \alpha n \rceil} g(y_{\lceil \alpha n \rceil}) - g(y_i) \right\}
\end{align}
which we also call the $g,\alpha$-entropy $e_{\alpha}(\mathbf{y}_n)$ of the sample $\mathbf{y}_n$.
...
\end{align}
If $F_2 = F_{y_0}$ is deterministic, then from \eqref{eqn:entropy_determ} we have 
\begin{align}
d_{\alpha}(F_1, F_{y_0}) = S_{\alpha}(F_1, F_{y_0}) = E_{F_{y_0}}[S_{\alpha}(F_1, Y)] = S_{\alpha}(F_1, y_0) = s_{\alpha}(q_{\alpha, F_1}, y_0)
\end{align}
whereas if $F_1 = F_{y_0}$, we get
\begin{align}
d_{\alpha}(F_{y_0}, F_2) = E_{F_2}[s_{\alpha}(y_0,Y)] - e_{\alpha}(F_2).
\end{align}
When $F_2 = F_{\mathbf{y}_n}$ is an empirical distribution, then 
\begin{align}
d_{\alpha}(F_1, F_{\mathbf{y}_n}) 
&= E_{F_{\mathbf{y}_n}}[S_{\alpha}(F_1, Y)] - e_{\alpha}(\mathbf{y}_n) \\
&= \frac{1}{n}\sum_{i=1}^{n} s_{\alpha}(q_{\alpha,F_1}, y_i) - s_{\alpha}(y_{\lceil \alpha n \rceil}, y_i)
\end{align}
whereas inserting the $F_{\mathbf{y}_n}$ in the first position gives
\begin{align}
d_{\alpha}(F_{\mathbf{y}_n}, F_2) = E_{F_2}[s_{\alpha}(y_{\lceil \alpha n \rceil},Y)] - e_{\alpha}(F_2).
\end{align}

<!--
&= (1-\alpha)(g(q_{\alpha, F_1}) - g(y_{\lceil \alpha n \rceil})) \\
&+ \frac{1}{n} \left\{\sum_{i \in I_1} g(y_i) - g(q_{\alpha, F_1})
+\sum_{i \in I_2} g(q_{\alpha, F_1}) - g(y_i) 
+\sum_{i \in I_3} g(y_{\lceil \alpha n \rceil})) - g(q_{\alpha, F_1}) \right\}
 \text{ using \eqref{eqn:scalar_score_OR}}
where $I_1 = \{i | q_{\alpha, F_1} \leq y_i \leq y_{\lceil \alpha n \rceil}\}$, 
$I_2 = \{i | y_{\lceil \alpha n \rceil} \leq y_i \leq q_{\alpha, F_1}\}$, and 
$I_3 = \{i | y_{\lceil \alpha n \rceil}, q_{\alpha, F_1} \leq y_i\}$.
-->

As for entropy, we also can get more explicit forms for the divergence in terms of distributions $F_1, F_2$. 
From \eqref{eqn:scalar_obj_cdf_form} we get
\begin{align}
d_{\alpha}(F_1,F_2)
...
\end{align}
This offers a more explicit form of the divergence of a distribution from s sample $\mathbf{y}_n$:
\begin{align}
d_{\alpha}(F_1,F_{\mathbf{y}_n}) 
= g(q_{\alpha,F_1})\left(\frac{1}{n}\sum_{1=1}^n \mathbb{1}\{y_i \leq q_{\alpha,F_1}\} - \alpha\right) 
+ \frac{1}{n}\sum_{1=1}^{\lceil \alpha n \rceil} \mathbb{1}\{y_i > q_{\alpha,F_1}\}g(y_i).
\end{align}
...
S(F,y) &:= s(q_{\alpha, F}, y) \\
S(F_1,F_2) &:= E_{F_2}[S(F_1,Y)] = s(q_{\alpha, F_1}, F_2) \\
\overline{S}(F) &:= E_G[s(q_{ \alpha,F}, Y)]
\end{align} 
-->
...
\begin{align}
q_{O,U, F} = F^{-1}\left(\frac{U}{O+U}\right) = F^{-1}\left(\frac{p-c+r}{p-u + r}\right).
\end{align} 
...
To this end, let $Z_{\lambda,i}$ be the set of critical points in the $i$th coordinate corresponding to the value $\lambda$, that is,
the set
\begin{align}
Z_{\lambda,i} := \left\{x \in \mathbb{R} \cup \infty \mid 
\lambda \in -w_i^{-1}\partial\sbar_{F_i}(x) = w_i^{-1}\kappa_i g_i'(x)(\alpha_i - [F_i(x-), F_i(x)])\right\} \label{dual_comp_deriv}
\end{align}
of $x$ where the weighted subgradient of $-\sbar_{F_i}(x)$ contains 
$\lambda$ (c.f. \eqref{scalar_Z_deriv}). (Allowing $x=\infty$ accommodates the case when $\lambda=0$ and $\alpha_i = 1$.)
Since $\lambda w_i x + \sbar_{F,i}(x) \geq 0$ for $\lambda \geq 0$, we have
\begin{align}
...
and in this case we have $L_{D,i}(\lambda) = \sbar_{F_i}(0)$.
\end{itemize}
...
## Special Case I: $F_i$ from same location-scale family and $w_i = 1$, $\alpha_i = \alpha$.

Also assume the constraint is active, i.e., 
\begin{align}
K < \sum F_i^{-1}(\alpha)= \sum \mu_i + \sigma_i\Phi^{-1}(\alpha)
\end{align}
where $\Phi$ is the base cdf for the family. Then solving for $\lambda^{\star}$ in
\begin{align}
\sum F_i^{-1}(\alpha - \lambda^{\star}) = \sum \mu_i + \sigma_i\Phi^{-1}(\alpha - \lambda^{\star}) = K
\end{align}
we get
\begin{align}
\alpha - \lambda^{\star} = 
...
\end{align}
When $\alpha = 1$, we might interpet $F(K)$ as the forecast probability under an independence assumption that $K$ will be sufficient to meet demand.

We can now use the assumption that the $F_i$ have a common base distribution to express the Bayes act as
\begin{align} 
 \nonumber x_i^{F,K} = F_i^{-1}(F(K)) &= \mu_i +\sigma_i\Phi^{-1}(F(K)) \\ 
...
- The $x_i^{F,K}$ here do not depend on $\alpha$ unlike when the constraint is inactive or the $\alpha_i$ differ. Intuitively, when the distibutions $F_i$
all have the same intrinsic shape, a change in $\alpha$ changes the level of marginal expected benefit identically in each coordinate, so that an equality
of these expected benefits is maintained.
